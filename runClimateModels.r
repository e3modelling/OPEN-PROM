# ============================================================
# Climate Model (Emulator) Assessment Runner
# ============================================================

# This R script automates the process of running climate model assessments (using CICERO-SCM or MAGICC) on emissions scenarios, 
# comparing outputs with expected benchmark results, and visualizing global temperature projections. 
# It is designed to interface with the climate-assessment Python workflow, while handling input/output preparation and post-processing in R.

# Key features:
# Cross-platform path handling: Converts Windows paths to WSL-compatible ones (to_wsl_path), enabling mixed Windows/Linux workflows.
# Model configuration: Prepares model-specific settings, including: (config/probabilistic files, Python virtual environment, Workflow script paths)
# Workflow execution (runAssessment): Builds and runs the appropriate command to launch the Python-based climate assessment pipeline for CICERO-SCM or MAGICC.
# Result loading (loadResults): Reads model output (Excel) into R using readxl, quitte, and magclass.
# Visualization (visualizeOutput): Extracts median global surface temperature (GSAT) trajectories and 
#                                  Generates a line plot comparing scenario results with default/expected outputs.
# CLI integration: Uses optparse to define command-line arguments (--model, --runFolder, --emissions, --output). 
#                  Defaults to model = ciceroscm and runFolder = daily_npi if not specified.

# Typical workflow: 
# 1. Place emissions input files under the designated runs/<runFolder>/emissions.csv directory (or specify a custom path).
# 2. Run the script with model and folder options, e.g.: "Rscript run_assessment.R --model magicc --runFolder daily_npi"
# 3. The script will:
# 3.1 Call the Python workflow inside the correct virtual environment.
# 3.2 Save processed outputs to runs/<runFolder>/EmissionsOutput-<model>/.
# 3.3 Compare against expected outputs and generate a global mean temperature plot (Global_Mean_Temperature.png).

# Dependencies: 
# R packages: optparse, readxl, dplyr, ggplot2, stringr, magclass, quitte
# Python environment: Installed CICERO-SCM or MAGICC workflow under climate-assessment/
...
if (is.null(opt$model) || is.null(opt$`run-folder`)) {
  ...
  model <- "magicc"
  runFolder <- "SoCDR_Ed3_HighestAmbition_0.135_0.075_CP1017_2100_2026-02-18_11-26-55"
} else {
  model <- opt$model
