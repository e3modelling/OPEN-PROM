# 02 Task Runner settings in VS code
...
At this stage, you do not need to memorize every task. The important point is that different buttons trigger different combinations of GAMS flags via `start.R` + the task body in `scripts/tasks/`, such as `DevMode`, `GenerateInput`, and reporting options.
