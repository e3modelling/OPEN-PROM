...
if (task == 0) {
  ...
  if (.Platform$OS.type == "unix") {
    cmdCommand <- paste0(gams, " main.gms ", gmsCmdOpt(cfg), " --GenerateInput=on -logOption 4 -Idir=./data 2>&1")
    system(cmdCommand)
  } else {
    cmdCommand <- paste0(gams, " main.gms ", gmsCmdOpt(cfg), " --GenerateInput=on -logOption 4 -Idir=./data 2>&1 | tee full.log")
    shell(cmdCommand)
  }
  if (withRunFolder) {
  ...
  if (.Platform$OS.type == "unix") {
    cmdCommand <- paste0(gams, " main.gms ", gmsCmdOpt(cfg), " --GenerateInput=off -logOption 4 -Idir=./data 2>&1")
    system(cmdCommand)
  } else {
    cmdCommand <- paste0(gams, " main.gms ", gmsCmdOpt(cfg), " --GenerateInput=off -logOption 4 -Idir=./data 2>&1 | tee full.log")
    shell(cmdCommand)
  }
