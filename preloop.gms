...
VLongAvgPowGenCost.L(runCy,ESET,"%fStartHorizon%") = 0;
VLongAvgPowGenCost.L(runCy,ESET,"%fBaseY%") = 0;
vAvgPowerGenCostShoTrm.L(runCy,ESET,"%fStartHorizon%") = 0;
vAvgPowerGenCostShoTrm.L(runCy,ESET,"%fBaseY%") = 0;
VLonAvgPowGenCostNoClimPol.L(runCy,PGALL,ESET,"%fStartHorizon%") = 0;
VLonPowGenCostNoClimPol.L(runCy,ESET,"%fBaseY%") = 0;
VLonPowGenCostNoClimPol.L(runCy,ESET,"%fStartHorizon%") = 0;
VLongPowGenCost.L(runCy,PGALL,ESET,"%fStartHorizon%") = 0;
...
VMExtF.L(runCy,YTIME)$((not An(YTIME)) $(ord(YTIME) gt 1)  ) = ( iTransChar(runCy,"RES_MEXTF",YTIME) * iSigma(runCy,"S1") * EXP(iSigma(runCy,"S2") *
           EXP(iSigma(runCy,"S3") * VLamda.L(runCy,YTIME))) * VNumVeh.L(runCy,YTIME-1) /(iPop(YTIME-1,runCy) * 1000) )$(iPop(YTIME-1,runCy));
...
VMExtF.FX(runCy,YTIME)$((not An(YTIME)) $(ord(YTIME) gt 1)  ) = ( iTransChar(runCy,"RES_MEXTF",YTIME) * iSigma(runCy,"S1") * EXP(iSigma(runCy,"S2") * EXP(iSigma(runCy,"S3") *
                          VLamda.l(runCy,YTIME)))* VNumVeh.l(runCy,YTIME-1) /(iPop(YTIME-1,runCy) * 1000) )$(iPop(YTIME-1,runCy))+VMExtF.l(runCy,YTIME-1)$(not iPop(YTIME-1,runCy));
...
* FIXME: VConsEachTechTransp.FX(runCy,TRANSE,TTECH,EF,YTIME)... , only the line of code below
* author=giannou
...
VFuePriSubChp.FX(runCy,DSBS,EF,YTIME)$((not An(YTIME)) $(not TRANSE(DSBS))  $SECTTECH(DSBS,EF)) =
   ...
   (0$(not CHP(EF)) + (VFuelPriceSub.l(runCy,"OI","ELC",YTIME)*iFracElecPriChp*iElecIndex(runCy,"%fStartHorizon%"))$CHP(EF))) + (0.003) + 
   ...
   (VFuelPriceSub.l(runCy,"OI","ELC",YTIME)*iFracElecPriChp*iElecIndex(runCy,"%fStartHorizon%"))$CHP(EF)))-(0.003)) + SQR(1e-7) ) )/2;
...
VNewCapYearly.FX(runCy,PGALL,"2011")$PGREN(PGALL) = iInstCapPast(runCy,PGALL,"2011")- iInstCapPast(runCy,PGALL,"%fStartHorizon%") +1E-10;
...
VNewCapYearly.FX(runCy,PGALL,"%fBaseY%")$PGREN(PGALL) = iInstCapPast(runCy,PGALL,"%fBaseY%")- iInstCapPast(runCy,PGALL,"2016") +1E-10;
