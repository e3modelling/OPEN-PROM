...
```jsonc
{
  ...
  "description":   "UPTAKE C600 landHigh iterated to convergence",  // from CSV
  ...
  "magpie": {
    ...
    "max_iter":          5,                                         // CSV overrides 1 -> 5
    "price_tol":         0.05,                                      // CSV matched default
    "quant_tol":         0.05                                       // CSV matched default
  }
}
```
...
* **Maturity factors work the same way.** `maturity_factors.changes.<name>.<field>` overrides one field of a change defined in `config.json` (a column cannot add new changes), and `maturity_factors.levels.<name>` redefines a level. See Tutorial 15, section 5.
