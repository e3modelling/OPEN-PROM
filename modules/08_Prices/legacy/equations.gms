...
Q08PriceFuelSubsecCarVal(allCy,SBS,EF,YTIME)$(SECTTECH(SBS,EF) $TIME(YTIME) $(not sameas("NUC",EF)) $runCy(allCy))..
         ...
         +
         (
           ( VMPriceElecIndResConsu(allCy,"i",YTIME)$INDTRANS(SBS)+VMPriceElecIndResConsu(allCy,"r",YTIME)$RESIDENT(SBS))/sTWhToMtoe
            +
...
Q08PriceElecIndResConsu(allCy,ESET,YTIME)$(TIME(YTIME)$(runCy(allCy)))..  !! The electricity price is based on previous year's production costs
        ...
        (
           (
             (VMPriceFuelSubsecCarVal(allCy,"OI","ELC",YTIME-1)*sTWhToMtoe)$TFIRST(YTIME-1) +
             (
        ...
        +
           (
             (VMPriceFuelSubsecCarVal(allCy,"HOU","ELC",YTIME-1)*sTWhToMtoe)$TFIRST(YTIME-1) +
             (
...
Q08PriceFuelSubsecCHP(allCy,DSBS,EF,YTIME)$(TIME(YTIME) $(not TRANSE(DSBS))  $SECTTECH(DSBS,EF) $runCy(allCy))..
        VMPriceFuelSubsecCHP(allCy,DSBS,EF,YTIME)
                =E=   
             (((VMPriceFuelSubsecCarVal(allCy,DSBS,EF,YTIME) + (MVRenValue(YTIME)/1000)$(not RENEF(EF))+iVarCostTech(allCy,DSBS,EF,YTIME)/1000)/iUsfEneConvSubTech(allCy,DSBS,EF,YTIME)- 
               (0$(not CHP(EF)) + (VMPriceFuelSubsecCarVal(allCy,"OI","ELC",YTIME)*sFracElecPriChp*VMPriceElecInd(allCy,YTIME))$CHP(EF)))  + SQRT( SQR(((VMPriceFuelSubsecCarVal(allCy,DSBS,EF,YTIME)+iVarCostTech(allCy,DSBS,EF,YTIME)/1000)/iUsfEneConvSubTech(allCy,DSBS,EF,YTIME)- 
              (0$(not CHP(EF)) + (VMPriceFuelSubsecCarVal(allCy,"OI","ELC",YTIME)*sFracElecPriChp*VMPriceElecInd(allCy,YTIME))$CHP(EF))))  ) )/2;
