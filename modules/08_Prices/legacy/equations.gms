...
$ENDIF
   $(not sameas("NUC",EFS)) $runCy(allCy))..
    ...
    (1 + ((((imFuelPrice(allCy,SBS,"CRO",YTIME) + SQRT(SQR(imFuelPrice(allCy,SBS,"CRO",YTIME)) + 1e-8))/2 + 1e-6) / (((imFuelPrice(allCy,SBS,"CRO",YTIME-1) + SQRT(SQR(imFuelPrice(allCy,SBS,"CRO",YTIME-1)) + 1e-8))/2) + 1e-6)) ** 0.4 - 1)$sameas("NGS",EFS)) *
    (1 + ((((imFuelPrice(allCy,SBS,"CRO",YTIME) + SQRT(SQR(imFuelPrice(allCy,SBS,"CRO",YTIME)) + 1e-8))/2 + 1e-6) / (((imFuelPrice(allCy,SBS,"CRO",YTIME-1) + SQRT(SQR(imFuelPrice(allCy,SBS,"CRO",YTIME-1)) + 1e-8))/2) + 1e-6)) ** 0.8 - 1)$SECtoEFPROD("LQD",EFS)) +
    (
