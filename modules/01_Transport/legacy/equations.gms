...
Q01GapTranspActiv(allCy,TRANSE,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         ...
         (sum((EF)$SECTTECH(TRANSE,EF),MVLft(allCy,TRANSE,EF,YTIME-1))/TECHS(TRANSE))] + SQRT( SQR([V01ActivGoodsTransp(allCy,TRANSE,YTIME) - V01ActivGoodsTransp(allCy,TRANSE,YTIME-1) + V01ActivGoodsTransp(allCy,TRANSE,YTIME-1)/
...
Q01CostTranspPerMeanConsSize(allCy,TRANSE,RCon,TTECH,YTIME)$(TIME(YTIME) $SECTTECH(TRANSE,TTECH) $(ord(Rcon) le iNcon(TRANSE)+1) $runCy(allCy))..
         ...
         =E=
                       (
                         ...
                         (
                           (sum(EF$TTECHtoEF(TTECH,EF),V01ConsSpecificFuel(allCy,TRANSE,TTECH,EF,YTIME)*MVPriceFuelSubsecCarVal(allCy,TRANSE,EF,YTIME)) )$(not PLUGIN(TTECH))
                           +
                           (sum(EF$(TTECHtoEF(TTECH,EF) $(not sameas("ELC",EF))),

                              (1-iShareAnnMilePlugInHybrid(allCy,YTIME))*V01ConsSpecificFuel(allCy,TRANSE,TTECH,EF,YTIME)*MVPriceFuelSubsecCarVal(allCy,TRANSE,EF,YTIME))
                             + iShareAnnMilePlugInHybrid(allCy,YTIME)*V01ConsSpecificFuel(allCy,TRANSE,TTECH,"ELC",YTIME)*MVPriceFuelSubsecCarVal(allCy,TRANSE,"ELC",YTIME)
                           )$PLUGIN(TTECH)
...
Q01CostTranspPerVeh(allCy,TRANSE,rCon,TTECH,YTIME)$(TIME(YTIME) $SECTTECH(TRANSE,TTECH) $(ord(rCon) le iNcon(TRANSE)+1) $runCy(allCy))..
...
Q01CostTranspMatFac(allCy,TRANSE,RCon,TTECH,YTIME)$(TIME(YTIME) $SECTTECH(TRANSE,TTECH) $(ord(rCon) le iNcon(TRANSE)+1) $runCy(allCy))..
...
Q01TechSortVarCost(allCy,TRANSE,rCon,YTIME)$(TIME(YTIME) $(ord(rCon) le iNcon(TRANSE)+1) $runCy(allCy))..
...
Q01ShareTechTr(allCy,TRANSE,TTECH,YTIME)$(TIME(YTIME) $SECTTECH(TRANSE,TTECH) $runCy(allCy))..
         ...
         * sum( Rcon$(ord(Rcon) le iNcon(TRANSE)+1),
                V01CostTranspPerVeh(allCy,TRANSE,RCon,TTECH,YTIME)
                * iDisFunConSize(allCy,TRANSE,RCon) / V01TechSortVarCost(allCy,TRANSE,RCon,YTIME)
              );
...
Q01ConsTechTranspSectoral(allCy,TRANSE,TTECH,EF,YTIME)$(TIME(YTIME) $SECTTECH(TRANSE,TTECH) $TTECHtoEF(TTECH,EF) $runCy(allCy))..
         ...
         (
                 ...
                 +
                 ( ((1-iShareAnnMilePlugInHybrid(allCy,YTIME))*V01ConsSpecificFuel(allCy,TRANSE,TTECH,EF,YTIME))$(not sameas("ELC",EF))
                   + iShareAnnMilePlugInHybrid(allCy,YTIME)*V01ConsSpecificFuel(allCy,TRANSE,TTECH,"ELC",YTIME))$PLUGIN(TTECH)
         )/1000
         * V01GapTranspActiv(allCy,TRANSE,YTIME) *
         (
                 (
...
Q01MEPcGdp(allCy,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         V01MEPcGdp(allCy,YTIME)
                 =E=
         iTransChar(allCy,"RES_MEXTV",YTIME) * V01MEPcGdp(allCy,YTIME-1) *
...
Q01MEPcNonGdp(allCy,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         V01MEPcNonGdp(allCy,YTIME)
                 =E=
         iTransChar(allCy,"RES_MEXTF",YTIME) * iSigma(allCy,"S1") * EXP(iSigma(allCy,"S2") * EXP(iSigma(allCy,"S3") * V01PcOwnPcLevl(allCy,YTIME))) *
         V01StockPcYearly(allCy,YTIME-1) / (iPop(YTIME-1,allCy) * 1000);
...
Q01StockPcYearly(allCy,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         V01StockPcYearly(allCy,YTIME)
                 =E=
         (V01StockPcYearly(allCy,YTIME-1)/(iPop(YTIME-1,allCy)*1000) + V01MEPcNonGdp(allCy,YTIME) + V01MEPcGdp(allCy,YTIME)) *
...
Q01NewRegPcYearly(allCy,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         V01NewRegPcYearly(allCy,YTIME)
                 =E=
         (V01MEPcNonGdp(allCy,YTIME) + V01MEPcGdp(allCy,YTIME)) * (iPop(YTIME,allCy)*1000)  !! new cars due to GDP
         - V01StockPcYearly(allCy,YTIME-1)*(1 - iPop(YTIME,allCy)/iPop(YTIME-1,allCy))    !! new cars due to population
...
Q01ActivPassTrnsp(allCy,TRANSE,YTIME)$(TIME(YTIME) $TRANP(TRANSE) $runCy(allCy))..
         ...
         (  !! passenger cars
           ...
           [(V01StockPcYearly(allCy,YTIME-1)/(iPop(YTIME-1,allCy)*1000))/(V01StockPcYearly(allCy,YTIME)/(iPop(YTIME,allCy)*1000))]**iElastA(allCy,TRANSE,"b3",YTIME) *
         ...
         (   !! other passenger transportation modes
           ...
           (MVPriceFuelAvgSub(allCy,TRANSE,YTIME)/MVPriceFuelAvgSub(allCy,TRANSE,YTIME-1))**iElastA(allCy,TRANSE,"c1",YTIME) *
           (MVPriceFuelAvgSub(allCy,TRANSE,YTIME-1)/MVPriceFuelAvgSub(allCy,TRANSE,YTIME-2))**iElastA(allCy,TRANSE,"c2",YTIME) *
           [(V01StockPcYearly(allCy,YTIME)*V01ActivPassTrnsp(allCy,"PC",YTIME))/(V01StockPcYearly(allCy,YTIME-1)*V01ActivPassTrnsp(allCy,"PC",YTIME-1))]**iElastA(allCy,TRANSE,"c4",YTIME) *
           prod(kpdl,
                  [(MVPriceFuelAvgSub(allCy,TRANSE,YTIME-ord(kpdl))/
                    MVPriceFuelAvgSub(allCy,TRANSE,YTIME-(ord(kpdl)+1)))/
                    (iCGI(allCy,YTIME)**(1/6))]**(iElastA(allCy,TRANSE,"c3",YTIME)*iFPDL(TRANSE,KPDL))
...
Q01PcOwnPcLevl(allCy,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         V01PcOwnPcLevl(allCy,YTIME) !! level of saturation of gompertz function
                 =E=
         ( (V01StockPcYearly(allCy,YTIME-1)/(iPop(YTIME-1,allCy)*1000)) / iPassCarsMarkSat(allCy) + 1 - SQRT( SQR((V01StockPcYearly(allCy,YTIME-1)/(iPop(YTIME-1,allCy)*1000)) /  iPassCarsMarkSat(allCy) - 1)  ) )/2;
