...
i01StockPcYearly(runCy,YTIME)$(not An(YTIME)) = imActv(YTIME,runCy,"PC");
*---
...
i01ActivPassTrnsp(runCy,"PC",YTIME)$DATAY(YTIME) = imTransChar(runCy,"KM_VEH",YTIME);
i01ActivPassTrnsp(runCy,TRANP,YTIME)$(DATAY(YTIME) and not sameas(TRANP,"PC")) = imActv(YTIME,runCy,TRANP);
*---
...
i01RateScrPc(runCy,TTECH,YTIME)$(not SECTTECH("PC",TTECH)) = 0;
i01RateScrPc(runCy,TTECH,YTIME)$(DATAY(YTIME) and SECTTECH("PC",TTECH)) = 1 / i01TechLft(runCy,"PC",TTECH,YTIME);
imLft(runCy,"PC",TTECH,YTIME)$DATAY(YTIME) = i01TechLft(runCy,"PC",TTECH,YTIME);
imLft(runCy,DSBS,TECH,YTIME)$(SECTTECH(DSBS,TECH) and (not sameas(DSBS,"PC"))) = i01TechLft(runCy,DSBS,TECH,YTIME);
imLft(runCy,DSBS,TECH,YTIME)$(not SECTTECH(DSBS,TECH)) = 0;
*---
...
i01StockPcYearlyTech(runCy,TTECH,YTIME)$DATAY(YTIME) = i01StockPC(runCy,TTECH,YTIME);
i01StockPcYearlyTech(runCy,TTECH,YTIME)$(not SECTTECH("PC",TTECH)) = 0;
*---
...
i01ActivGoodsTransp(runCy,TRANG,YTIME)$(not An(YTIME)) = imActv(YTIME,runCy,TRANG);
i01ActivGoodsTransp(runCy,TRANSE,YTIME)$(not TRANG(TRANSE)) = 0;
*---
...
i01PcOwnPcLevl(runCy,YTIME)$(not An(YTIME)) = V01StockPcYearly.L(runCy,YTIME) / (i01Pop(YTIME,runCy) * 1000) ;
*---
...
i01ConsTechTranspSectoral(runCy,TRANSE,TTECH,EF,YTIME)$DATAY(YTIME) = V01ConsTechTranspSectoral.L(runCy,TRANSE,TTECH,EF,YTIME);
*---
...
i01CostFuel(runCy,TRANSE,TTECH,YTIME)$DATAY(YTIME) = V01CostFuel.L(runCy,TRANSE,TTECH,YTIME);
*---
...
i01CostTranspPerMeanConsSize(runCy,TRANSE,TTECH,YTIME)$DATAY(YTIME) = V01CostTranspPerMeanConsSize.L(runCy,TRANSE,TTECH,YTIME);
*---
