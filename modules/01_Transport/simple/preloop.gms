...
p01PremScrp(runCy,TRANSE,TTECH,YTIME)$(not SECTTECH(TRANSE,TTECH)) = 0;
*---
V01StockPcYearly.L(runCy,YTIME) = 0.1;
V01StockPcYearly.FX(runCy,YTIME)$(not An(YTIME)) = imActv(YTIME,runCy,"PC");
p01StockPcYearly(runCy,YTIME)$(not An(YTIME)) = imActv(YTIME,runCy,"PC");
*---
...
p01RateScrPc(runCy,TTECH,YTIME)$(not SECTTECH("PC",TTECH)) = 0;
p01RateScrPc(runCy,TTECH,YTIME)$(DATAY(YTIME) and SECTTECH("PC",TTECH)) = 1 / i01TechLft(runCy,"PC",TTECH,YTIME);
pmLft(runCy,"PC",TTECH,YTIME)$DATAY(YTIME) = i01TechLft(runCy,"PC",TTECH,YTIME);
pmLft(runCy,DSBS,TECH,YTIME)$(SECTTECH(DSBS,TECH) and (not sameas(DSBS,"PC"))) = i01TechLft(runCy,DSBS,TECH,YTIME);
pmLft(runCy,DSBS,TECH,YTIME)$(not SECTTECH(DSBS,TECH)) = 0;
*---
V01RateScrPcTot.FX(runCy,TTECH,YTIME)$DATAY(YTIME) = V01RateScrPc.L(runCy,TTECH,YTIME);
p01RateScrPcTot(runCy,TTECH,YTIME)$DATAY(YTIME) = V01RateScrPc.L(runCy,TTECH,YTIME);
*---
...
p01StockPcYearlyTech(runCy,TTECH,YTIME)$DATAY(YTIME) = i01StockPC(runCy,TTECH,YTIME);
p01StockPcYearlyTech(runCy,TTECH,YTIME)$(not SECTTECH("PC",TTECH)) = 0;
*---
...
p01NumPcScrap(runCy,YTIME)$sameas(YTIME,"%fBaseY%") = SUM(TTECH,V01RateScrPcTot.L(runCy,TTECH,YTIME) * V01StockPcYearlyTech.L(runCy,TTECH,YTIME));
*---
...
p01ActivGoodsTransp(runCy,TRANG,YTIME)$(not An(YTIME)) = imActv(YTIME,runCy,TRANG);
p01ActivGoodsTransp(runCy,TRANSE,YTIME)$(not TRANG(TRANSE)) = 0;
*---
...
p01PcOwnPcLevl(runCy,YTIME)$(not An(YTIME)) = V01StockPcYearly.L(runCy,YTIME) / (i01Pop(YTIME,runCy) * 1000) ;
*---
...
p01ConsTechTranspSectoral(runCy,TRANSE,TTECH,EF,YTIME)$DATAY(YTIME) = V01ConsTechTranspSectoral.L(runCy,TRANSE,TTECH,EF,YTIME);
*---
...
p01CostFuel(runCy,TRANSE,TTECH,YTIME)$DATAY(YTIME) = V01CostFuel.L(runCy,TRANSE,TTECH,YTIME);
*---
...
p01CostTranspPerMeanConsSize(runCy,TRANSE,TTECH,YTIME)$DATAY(YTIME) = V01CostTranspPerMeanConsSize.L(runCy,TRANSE,TTECH,YTIME);
*---
