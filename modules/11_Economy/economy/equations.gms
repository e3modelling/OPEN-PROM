          ...
          (
            (
              sum((EF,EFS)$EFtoEFS(EF,EFS),
                pmConsFinEneCountry(allCy,EF,YTIME-1) * imCo2EmiFac(allCy,"PG",EF,YTIME-1)) 
            ...
            +
            sqrt(sqr(
              sum((EF,EFS)$EFtoEFS(EF,EFS),pmConsFinEneCountry(allCy,EF,YTIME-1) * imCo2EmiFac(allCy,"PG",EF,YTIME-1))
            + sum(SSBS,p07GrossEmissCO2Supply(allCy,SSBS,YTIME-1))
              -
              (p06CaptCummCO2(allCy,YTIME-1) - p06CaptCummCO2(allCy,YTIME-2))
            ))
...
$$offtext 
    sum(CDRTECH$(sameas(TECH,CDRTECH)), !! CDR
      ...
      (p06CapCDR(allCy,CDRTECH,YTIME-1) * p06CapFacNewDAC(allCy,CDRTECH,YTIME-1))
