...
$ontext
VProdElec.FX(runCyL,PGALL,YTIME)$TIME(YTIME) = VProdElec.L(runCyL,PGALL,YTIME)$TIME(YTIME);
VScalFacPlaDisp.FX(runCyL,HOUR,YTIME)$TIME(YTIME) = VScalFacPlaDisp.L(runCyL,HOUR,YTIME)$TIME(YTIME);
VProdElecReqTot.FX(runCyL,YTIME)$TIME(YTIME) = VProdElecReqTot.L(runCyL,YTIME)$TIME(YTIME);
VProdElecReqCHP.FX(runCyL,YTIME)$TIME(YTIME) = VProdElecReqCHP.L(runCyL,YTIME)$TIME(YTIME);
VSortPlantDispatch.FX(runCyL,PGALL,YTIME)$TIME(YTIME) = VSortPlantDispatch.L(runCyL,PGALL,YTIME)$TIME(YTIME);
VCapElecCHP.FX(runCyL,CHP,YTIME)$TIME(YTIME) = VCapElecCHP.L(runCyL,CHP,YTIME)$TIME(YTIME);
VDemElecTot.FX(runCyL,YTIME)$TIME(YTIME) = VDemElecTot.L(runCyL,YTIME)$TIME(YTIME);
$offtext