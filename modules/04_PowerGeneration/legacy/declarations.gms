...
Q04PotRenCurr(allCy,PGRENEF,YTIME)	                       "Compute current renewable potential"
...
Q04PotRenSuppCurve(allCy,PGRENEF,YTIME)	                   "Compute renewable potential supply curve"		
Q04PotRenMaxAllow(allCy,PGRENEF,YTIME)                     "Compute maximum allowed renewable potential"
...
Q04LoadFacDom(allCy,YTIME)	                               "Compute load factor of entire domestic system"
Q04DemElecTot(allCy,YTIME)                                 "Compute total electricity demand"
Q04ProdElecCHP(allCy,CHP,YTIME)	                           "Compute electricity production from CHP plants" 
Q04ProdElecReqTot(allCy,YTIME)                             "Compute total required electricity production"
...
V04PotRenCurr(allCy,PGRENEF,YTIME)	                       "Current renewable potential (GW)"			
...
V04PotRenSuppCurve(allCy,PGRENEF,YTIME)	                   "Renewable potential supply curve	(1)"
V04PotRenMaxAllow(allCy,PGRENEF,YTIME)                     "Maximum allowed renewable potential (GW)"
...
MVProdElec(allCy,PGALL,YTIME)                              "Electricity production (TWh)"	
MVCostPowGenAvgLng(allCy,ESET,YTIME)	                   "Long-term average power generation cost (US$2015/kWh)"
MVCapElecTotEst(allCy,YTIME)	                           "Estimated Total electricity generation capacity (GW)"
MVPeakLoad(allCy,YTIME)	                                   "Electricity peak load (GW)"	
MVCapElec(allCy,PGALL,YTIME)	                           "Electricity generation plants capacity (GW)"
MVBaseLoad(allCy,YTIME)	                                   "Corrected base load (GW)"
;