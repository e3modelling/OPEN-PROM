...
p04CapElecNominal(allCy,PGALL,YTIME)               "Stored nominal electricity generation plants capacity (GW)"
p04ShareTechPG(allCy,PGALL,YTIME)                  "Stored share of all technologies in the electricity mixture"
p04CostHourProdInvDec(allCy,PGALL,YTIME)           "Stored production cost of technology (US$2015/KWh)"
p04CostVarTech(allCy,PGALL,YTIME)                  "Stored variable cost of technology (US$2015/KWh)"
p04IndxEndogScrap(allCy,PGALL,YTIME)               "Stored endogenous scrapping index (1)"
p04CapElecNonCHP(allCy,YTIME)                      "Stored total electricity generation capacity excluding CHP (GW)"
p04GapGenCapPowerDiff(allCy,YTIME)                 "Stored gap in total generation capacity to be filled by new equipment (GW)"
p04ShareSatPG(allCy,PGALL,YTIME)                   "Stored saturation for electricity mixture penetration of RES technologies"
p04SharePowPlaNewEq(allCy,PGALL,YTIME)             "Stored power plant share in new equipment (1)"
p04SortPlantDispatch(allCy,PGALL,YTIME)            "Stored power plants sorting according to variable cost (1)"
p04NewCapElec(allCy,PGALL,YTIME)                   "Stored new capacity added every year (GW)"
p04NetNewCapElec(allCy,PGALL,YTIME)                "Stored yearly difference in installed capacity (MW)"
p04DemElecTot(allCy,YTIME)                         "Stored total electricity demand (TWh)"
p04ProdElecEstCHP(allCy,TCHP,YTIME)                "Stored estimated electricity of CHP plants (1)"
p04CapexFixCostPG(allCy,PGALL,YTIME)               "Stored CAPEX and fixed costs of any power generation technology (US$2015/kW)"
p04ShareMixWndSol(allCy,YTIME)                     "Stored participation of solar and wind in the energy mixture (%)"
p04CapexRESRate(allCy,PGALL,YTIME)                 "Stored RES capex rate"
p04CO2CaptRate(allCy,PGALL,YTIME)                  "Stored CO2 capture rate"
p04CostCapTech(allCy,PGALL,YTIME)                  "Stored capital cost of technology"
p04CCSRetroFit(allCy,PGALL,YTIME)                  "Stored CCS retrofit"
p04ScrpRate(allCy,PGALL,YTIME)                     "Stored scrapping rate"
pmProdElec(allCy,PGALL,YTIME)                      "Stored electricity production (TWh)"
pmCostPowGenAvgLng(allCy,YTIME)                    "Stored long-term average power generation cost (US$2015/kWh)"
pmCapElec(allCy,PGALL,YTIME)                       "Stored electricity generation plants capacity (GW)"
pmConsFuelElecProd(allCy,EFS,YTIME)                "Stored fuel consumption for electricity production"
;
