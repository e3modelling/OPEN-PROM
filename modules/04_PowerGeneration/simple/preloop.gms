...
*---
VmCapElec.LO(runCy,PGALL,YTIME) = epsilon6;
...
p04CapElecNominal(runCy,PGALL,YTIME) = V04CapElecNominal.L(runCy,PGALL,YTIME-1);
p04ShareTechPG(runCy,PGALL,YTIME) = V04ShareTechPG.L(runCy,PGALL,YTIME-1);
p04CostHourProdInvDec(runCy,PGALL,YTIME) = V04CostHourProdInvDec.L(runCy,PGALL,YTIME-1);
p04CostVarTech(runCy,PGALL,YTIME) = V04CostVarTech.L(runCy,PGALL,YTIME-1);
p04IndxEndogScrap(runCy,PGALL,YTIME) = V04IndxEndogScrap.L(runCy,PGALL,YTIME-1);
p04CapElecNonCHP(runCy,YTIME) = V04CapElecNonCHP.L(runCy,YTIME-1);
p04GapGenCapPowerDiff(runCy,YTIME) = V04GapGenCapPowerDiff.L(runCy,YTIME-1);
p04ShareSatPG(runCy,PGALL,YTIME) = V04ShareSatPG.L(runCy,PGALL,YTIME-1);
p04SharePowPlaNewEq(runCy,PGALL,YTIME) = V04SharePowPlaNewEq.L(runCy,PGALL,YTIME-1);
p04SortPlantDispatch(runCy,PGALL,YTIME) = V04SortPlantDispatch.L(runCy,PGALL,YTIME-1);
p04NewCapElec(runCy,PGALL,YTIME) = V04NewCapElec.L(runCy,PGALL,YTIME-1);
p04NetNewCapElec(runCy,PGALL,YTIME) = V04NetNewCapElec.L(runCy,PGALL,YTIME-1);
p04DemElecTot(runCy,YTIME) = V04DemElecTot.L(runCy,YTIME-1);
p04ProdElecEstCHP(runCy,TCHP,YTIME) = V04ProdElecEstCHP.L(runCy,TCHP,YTIME-1);
p04CapexFixCostPG(runCy,PGALL,YTIME) = V04CapexFixCostPG.L(runCy,PGALL,YTIME-1);
p04ShareMixWndSol(runCy,YTIME) = V04ShareMixWndSol.L(runCy,YTIME-1);
p04CapexRESRate(runCy,PGALL,YTIME) = V04CapexRESRate.L(runCy,PGALL,YTIME-1);
p04CO2CaptRate(runCy,PGALL,YTIME) = V04CO2CaptRate.L(runCy,PGALL,YTIME-1);
p04CostCapTech(runCy,PGALL,YTIME) = V04CostCapTech.L(runCy,PGALL,YTIME-1);
p04CCSRetroFit(runCy,PGALL,YTIME) = V04CCSRetroFit.L(runCy,PGALL,YTIME-1);
p04ScrpRate(runCy,PGALL,YTIME) = V04ScrpRate.L(runCy,PGALL,YTIME-1);
pmProdElec(runCy,PGALL,YTIME) = VmProdElec.L(runCy,PGALL,YTIME-1);
pmCostPowGenAvgLng(runCy,YTIME) = VmCostPowGenAvgLng.L(runCy,YTIME-1);
pmCapElec(runCy,PGALL,YTIME) = VmCapElec.L(runCy,PGALL,YTIME-1);
pmConsFuelElecProd(runCy,EFS,YTIME) = VmConsFuelElecProd.L(runCy,EFS,YTIME-1);
