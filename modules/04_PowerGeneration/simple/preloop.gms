...
p04CapElecNominal(runCy,PGALL,YTIME)$(DATAY(YTIME)) = V04CapElecNominal.L(runCy,PGALL,YTIME);
p04ShareTechPG(runCy,PGALL,YTIME)$(DATAY(YTIME) and sum(PGALL2, imInstCapPastNonCHP(runCy,PGALL2,YTIME))) = imInstCapPastNonCHP(runCy,PGALL,YTIME) / sum(PGALL2, imInstCapPastNonCHP(runCy,PGALL2,YTIME));
p04CostVarTech(runCy,PGALL,YTIME)$(DATAY(YTIME)) = V04CostVarTech.L(runCy,PGALL,YTIME);
p04CapexFixCostPG(runCy,PGALL,YTIME)$(DATAY(YTIME)) = V04CapexFixCostPG.L(runCy,PGALL,YTIME);
p04CostCapTech(runCy,PGALL,YTIME)$(DATAY(YTIME)) = V04CostCapTech.L(runCy,PGALL,YTIME);
p04CostHourProdInvDec(runCy,PGALL,YTIME)$(DATAY(YTIME)) = V04CostHourProdInvDec.L(runCy,PGALL,YTIME);
p04CapElecNonCHP(runCy,YTIME)$(DATAY(YTIME)) = V04CapElecNonCHP.L(runCy,YTIME);
p04NetNewCapElec(runCy,PGALL,YTIME)$(DATAY(YTIME)) = imInstCapPastNonCHP(runCy,PGALL,YTIME) - imInstCapPastNonCHP(runCy,PGALL,YTIME-1);
p04NewCapElec(runCy,PGALL,YTIME)$(DATAY(YTIME)) = 0;
p04GapGenCapPowerDiff(runCy,YTIME)$(DATAY(YTIME)) = 0;
p04IndxEndogScrap(runCy,PGALL,YTIME)$(DATAY(YTIME)) = 1;
p04ShareSatPG(runCy,PGALL,YTIME)$(DATAY(YTIME)) = 1;
p04CCSRetroFit(runCy,PGALL,YTIME)$(DATAY(YTIME)) = 1;
p04CO2CaptRate(runCy,PGALL,YTIME)$(DATAY(YTIME)) = 0;
p04ScrpRate(runCy,PGALL,YTIME)$(DATAY(YTIME)) = 0;
p04SharePowPlaNewEq(runCy,PGALL,YTIME)$(DATAY(YTIME)) = 0;
p04SortPlantDispatch(runCy,PGALL,YTIME)$(DATAY(YTIME)) = 1e-6;
p04CapexRESRate(runCy,PGALL,YTIME)$(DATAY(YTIME)) = 1;
p04ShareMixWndSol(runCy,YTIME)$(DATAY(YTIME)) = V04ShareMixWndSol.L(runCy,YTIME);
pmProdElec(runCy,PGALL,YTIME)$(DATAY(YTIME)) = i04DataElecProdNonCHP(runCy,PGALL,YTIME) / 1000;
