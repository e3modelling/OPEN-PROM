...
p04CostHourProdInvDec(runCy,PGALL,YTIME) = V04CostHourProdInvDec.L(runCy,PGALL,YTIME);
p04CostVarTech(runCy,PGALL,YTIME) = V04CostVarTech.L(runCy,PGALL,YTIME);
p04IndxEndogScrap(runCy,PGALL,YTIME) = V04IndxEndogScrap.L(runCy,PGALL,YTIME);
p04CapElecNonCHP(runCy,YTIME) = V04CapElecNonCHP.L(runCy,YTIME);
p04GapGenCapPowerDiff(runCy,YTIME) = V04GapGenCapPowerDiff.L(runCy,YTIME);
p04ShareSatPG(runCy,PGALL,YTIME) = V04ShareSatPG.L(runCy,PGALL,YTIME);
p04SharePowPlaNewEq(runCy,PGALL,YTIME) = V04SharePowPlaNewEq.L(runCy,PGALL,YTIME);
p04SortPlantDispatch(runCy,PGALL,YTIME) = V04SortPlantDispatch.L(runCy,PGALL,YTIME);
p04NewCapElec(runCy,PGALL,YTIME) = V04NewCapElec.L(runCy,PGALL,YTIME);
p04NetNewCapElec(runCy,PGALL,YTIME) = V04NetNewCapElec.L(runCy,PGALL,YTIME);
p04DemElecTot(runCy,YTIME) = V04DemElecTot.L(runCy,YTIME);
p04ProdElecEstCHP(runCy,TCHP,YTIME) = V04ProdElecEstCHP.L(runCy,TCHP,YTIME);
p04CapexFixCostPG(runCy,PGALL,YTIME) = V04CapexFixCostPG.L(runCy,PGALL,YTIME);
p04ShareMixWndSol(runCy,YTIME) = V04ShareMixWndSol.L(runCy,YTIME);
p04CapexRESRate(runCy,PGALL,YTIME) = V04CapexRESRate.L(runCy,PGALL,YTIME);
p04CO2CaptRate(runCy,PGALL,YTIME) = V04CO2CaptRate.L(runCy,PGALL,YTIME);
p04CostCapTech(runCy,PGALL,YTIME) = V04CostCapTech.L(runCy,PGALL,YTIME);
p04CCSRetroFit(runCy,PGALL,YTIME) = V04CCSRetroFit.L(runCy,PGALL,YTIME);
p04ScrpRate(runCy,PGALL,YTIME) = V04ScrpRate.L(runCy,PGALL,YTIME);
pmProdElec(runCy,PGALL,YTIME) = VmProdElec.L(runCy,PGALL,YTIME);
pmCostPowGenAvgLng(runCy,YTIME) = VmCostPowGenAvgLng.L(runCy,YTIME);
pmCapElec(runCy,PGALL,YTIME) = VmCapElec.L(runCy,PGALL,YTIME);
pmConsFuelElecProd(runCy,EFS,YTIME) = VmConsFuelElecProd.L(runCy,EFS,YTIME);
