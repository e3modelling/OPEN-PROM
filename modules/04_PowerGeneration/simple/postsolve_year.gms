...
V04CapElecNominal.L(runCyL,PGALL,YTIME+1) = p04CapElecNominal(runCy,PGALL,YTIME);
V04ShareTechPG.L(runCyL,PGALL,YTIME+1) = p04ShareTechPG(runCy,PGALL,YTIME);
V04CostHourProdInvDec.L(runCyL,PGALL,YTIME+1) = p04CostHourProdInvDec(runCy,PGALL,YTIME);
V04CostVarTech.L(runCyL,PGALL,YTIME+1) = p04CostVarTech(runCy,PGALL,YTIME);
V04IndxEndogScrap.L(runCyL,PGALL,YTIME+1) = p04IndxEndogScrap(runCy,PGALL,YTIME);
V04CapElecNonCHP.L(runCyL,YTIME+1) = p04CapElecNonCHP(runCy,YTIME);
V04GapGenCapPowerDiff.L(runCyL,YTIME+1) = p04GapGenCapPowerDiff(runCy,YTIME);
V04ShareSatPG.L(runCyL,PGALL,YTIME+1) = p04ShareSatPG(runCy,PGALL,YTIME);
V04SharePowPlaNewEq.L(runCyL,PGALL,YTIME+1) = p04SharePowPlaNewEq(runCy,PGALL,YTIME);
V04NewCapElec.L(runCyL,PGALL,YTIME+1) = p04NewCapElec(runCy,PGALL,YTIME);
V04NetNewCapElec.L(runCyL,PGALL,YTIME+1) = p04NetNewCapElec(runCy,PGALL,YTIME);
V04DemElecTot.L(runCyL,YTIME+1) = p04DemElecTot(runCy,YTIME);
V04ProdElecEstCHP.L(runCyL,TCHP,YTIME+1) = p04ProdElecEstCHP(runCy,TCHP,YTIME);
V04CapexFixCostPG.L(runCyL,PGALL,YTIME+1) = p04CapexFixCostPG(runCy,PGALL,YTIME);
V04ShareMixWndSol.L(runCyL,YTIME+1) = p04ShareMixWndSol(runCy,YTIME);
V04CapexRESRate.L(runCyL,PGALL,YTIME+1) = p04CapexRESRate(runCy,PGALL,YTIME);
V04CO2CaptRate.L(runCyL,PGALL,YTIME+1) = p04CO2CaptRate(runCy,PGALL,YTIME);
V04CostCapTech.L(runCyL,PGALL,YTIME+1) = p04CostCapTech(runCy,PGALL,YTIME);
V04CCSRetroFit.L(runCyL,PGALL,YTIME+1) = p04CCSRetroFit(runCy,PGALL,YTIME);
V04ScrpRate.L(runCyL,PGALL,YTIME+1) = p04ScrpRate(runCy,PGALL,YTIME);
VmProdElec.L(runCyL,PGALL,YTIME+1) = pmProdElec(runCy,PGALL,YTIME);
VmCostPowGenAvgLng.L(runCyL,YTIME+1) = pmCostPowGenAvgLng(runCy,YTIME);
VmCapElec.L(runCyL,PGALL,YTIME+1) = pmCapElec(runCy,PGALL,YTIME);
VmConsFuelElecProd.L(runCyL,EFS,YTIME+1) = pmConsFuelElecProd(runCy,EFS,YTIME);