...
$offtext
V02DemSubUsefulSubsec.FX(runCy,DSBS,YTIME)$(DATAY(YTIME) and not TRANSE(DSBS) and not sameas(DSBS,"DAC")) = [
      ...
      (VmPriceFuelAvgSub.L(runCy,DSBS,YTIME-1)/VmPriceFuelAvgSub.L(runCy,DSBS,YTIME-2) ) ** imElastA(runCy,DSBS,"b2",YTIME) *
      prod(KPDL,
        ((VmPriceFuelAvgSub.L(runCy,DSBS,YTIME-ord(KPDL))/VmPriceFuelAvgSub.L(runCy,DSBS,YTIME-(ord(KPDL)+1)))/(imCGI(runCy,YTIME)**(1/6)))**( imElastA(runCy,DSBS,"c",YTIME)*imFPDL(DSBS,KPDL))
      )
    ]$imActv(YTIME-1,runCy,DSBS)
;
 
 V02DemSubUsefulSubsec.FX(runCy,DSBS,YTIME)$(not(DATAY(YTIME) and not TRANSE(DSBS) and not sameas(DSBS,"DAC"))) = 0;
