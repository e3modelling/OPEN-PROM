...
V02EquipCapTechSubsec.FX(runCy,DSBS,ITECH,YTIME)$(SECTTECH(DSBS,ITECH) and not An(YTIME) and not TRANSE(DSBS) and sameas(ITECH,"TASHP")) = i02ShareElcHP(runCy,DSBS,YTIME) * i02ShareElcHP(runCy,ITECH,YTIME) * imFuelCons(runCy,DSBS,"ELC",YTIME) * (1-imShrNonSubElecInTotElecDem(runCy,DSBS));
V02EquipCapTechSubsec.FX(runCy,DSBS,ITECH,YTIME)$(SECTTECH(DSBS,ITECH) and not An(YTIME) and not TRANSE(DSBS) and sameas(ITECH,"TWSHP")) = i02ShareElcHP(runCy,DSBS,YTIME) * i02ShareElcHP(runCy,ITECH,YTIME) * imFuelCons(runCy,DSBS,"ELC",YTIME) * (1-imShrNonSubElecInTotElecDem(runCy,DSBS));
V02EquipCapTechSubsec.FX(runCy,DSBS,ITECH,YTIME)$(SECTTECH(DSBS,ITECH) and not An(YTIME) and not TRANSE(DSBS) and sameas(ITECH,"TGSHP")) = i02ShareElcHP(runCy,DSBS,YTIME) * i02ShareElcHP(runCy,ITECH,YTIME) * imFuelCons(runCy,DSBS,"ELC",YTIME) * (1-imShrNonSubElecInTotElecDem(runCy,DSBS));
...
V02VarCostTech.FX(runCy,DSBS,ITECH,YTIME)$(DATAY(YTIME) and (INDDOM(DSBS) or NENSE(DSBS)) and SECTTECH(DSBS,ITECH)) =
  ...
  ) / (imUsfEneConvSubTech(runCy,DSBS,ITECH,YTIME) + 1e-6);
...
V02CapCostTech.FX(runCy,DSBS,ITECH,YTIME)$(not An(YTIME) and (INDDOM(DSBS) or NENSE(DSBS)) and SECTTECH(DSBS,ITECH)) = ((
      ...
      (exp((imDisc(runCy,DSBS,YTIME)$(not TSTEAM(ITECH)) + imDisc(runCy,"PG",YTIME)$TSTEAM(ITECH)) * VmLft.L(runCy,DSBS,ITECH,YTIME)) - 1 +1e-6)
    ) *
