...
Q02DemSubUsefulSubsec(allCy,DSBS,YTIME)$(TIME(YTIME)$(not TRANSE(DSBS))$runCy(allCy))..
...
Q02GapUsefulDemSubsec(allCy,DSBS,YTIME)$(TIME(YTIME) $(not TRANSE(DSBS)) $runCy(allCy))..
        V02GapUsefulDemSubsec(allCy,DSBS,YTIME) =E=
        (V02DemSubUsefulSubsec(allCy,DSBS,YTIME) - V02DemUsefulSubsecRemTech(allCy,DSBS,YTIME)
        + SQRT(SQR(V02DemSubUsefulSubsec(allCy,DSBS,YTIME) - V02DemUsefulSubsecRemTech(allCy,DSBS,YTIME))))/2;



*' The equation computes the technology cost, including the lifetime factor, for each technology, and consumer size group
...
*' index, and useful energy conversion factors. ONLY CAPITAL COST COMPONENT
...
Q02ShareTechNewEquipUseful(allCy,DSBS,ITECH,YTIME)$(TIME(YTIME) $SECTTECH(DSBS,ITECH) $(not TRANSE(DSBS)) $runCy(allCy))..
        V02ShareTechNewEquipUseful(allCy,DSBS,ITECH,YTIME) =E=
