...
* Initialize variable levels (.L) 
* Now I fix 2019 as base year but this has to be changed with all the historical years
iProdHistIS(runCy,ISTECH_HIST,YTIME)$(YTIME eq '2019') = 0.0001; 
...
VCapFacIS.L(runCy,ISTECH_ALL,YTIME) = 0.8; 

* Initialize fuel consumption and emissions variables 
...
* Fix the available capacity of historical plants in the base year.
* The value is taken from the iInitialCapacityTotal parameter
VAvailCapISHist.FX(allCy, ISTECH_HIST, YTIME)$(YTIME.val = %fBaseY%) =
