...
V09ScrapRate.L(runCyL,TSTEAM,YTIME+1) = p09ScrapRate(runCy,TSTEAM,YTIME);
V09DemGapSte.L(runCyL,YTIME+1) = p09DemGapSte(runCy,YTIME);
V09GapShareSte.L(runCyL,TSTEAM,YTIME+1) = p09GapShareSte(runCy,TSTEAM,YTIME);
V09ScrapRatePremature.L(runCyL,TSTEAM,YTIME+1) = p09ScrapRatePremature(runCy,TSTEAM,YTIME);
V09CaptRateSte.L(runCyL,TSTEAM,YTIME+1) = p09CaptRateSte(runCy,TSTEAM,YTIME);
V09CostVarProdSte.L(runCyL,TSTEAM,YTIME+1) = p09CostVarProdSte(runCy,TSTEAM,YTIME);
V09CostCapProdSte.L(runCyL,TSTEAM,YTIME+1) = p09CostCapProdSte(runCy,TSTEAM,YTIME);
V09CostProdSte.L(runCyL,TSTEAM,YTIME+1) = p09CostProdSte(runCy,TSTEAM,YTIME);
VmDemTotSte.L(runCyL,YTIME+1) = pmDemTotSte(runCy,YTIME);
VmCapSte.L(runCyL,TSTEAM,YTIME+1) = pmCapSte(runCy,TSTEAM,YTIME);
VmProdSte.L(runCyL,TSTEAM,YTIME+1) = pmProdSte(runCy,TSTEAM,YTIME);
VmCostAvgProdSte.L(runCyL,YTIME+1) = pmCostAvgProdSte(runCy,YTIME);
VmConsFuelSteProd.L(runCyL,STEMODE,EFS,YTIME+1) = pmConsFuelSteProd(runCy,STEMODE,EFS,YTIME);
