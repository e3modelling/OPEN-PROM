...
Q09CapSte(allCy,TSTEAM,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
    VmCapSte(allCy,TSTEAM,YTIME)
        =E=
    (1-V09ScrapRate(allCy,TSTEAM,YTIME)) * VmCapSte(allCy,TSTEAM,YTIME-%fPeriodOfYears%) +
...
Q09DemGapSte(allCy,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
  ...
  ( 
      ...
      sum(TSTEAM,
        ...
        VmCapSte(allCy,TSTEAM,YTIME-%fPeriodOfYears%)
      ) +
  SQRT(SQR(
      VmDemTotSte(allCy,YTIME) -
      sum(TSTEAM,
...
Q09ScrapRatePremature(allCy,TSTEAM,YTIME)$(TIME(YTIME)$runCy(allCy))..
    V09ScrapRatePremature(allCy,TSTEAM,YTIME)
        =E=
    V09CostVarProdSte(allCy,TSTEAM,YTIME-%fPeriodOfYears%) ** (-2) /
    (
      V09CostVarProdSte(allCy,TSTEAM,YTIME-%fPeriodOfYears%) ** (-2) +
      (( 
        i09ScaleEndogScrap *
        sum(TCHP2$(not sameas(TSTEAM,TCHP2)),
          V09CostProdSte(allCy,TCHP2,YTIME-%fPeriodOfYears%)
        )
      ) ** (-2)
      )$TCHP(TSTEAM) +
      (( 
        ...
        sum(TDHP2$(not sameas(TSTEAM,TDHP2)),
          V09CostProdSte(allCy,TDHP2,YTIME-%fPeriodOfYears%)
        )
      ) ** (-2)
      )$TDHP(TSTEAM)
    );
