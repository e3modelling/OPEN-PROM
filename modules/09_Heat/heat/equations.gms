...
Q09CapSte(allCy,TSTEAM,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
    VmCapSte(allCy,TSTEAM,YTIME)
        =E=
    (1-V09ScrapRate(allCy,TSTEAM,YTIME)) * p09CapSte(allCy,TSTEAM,YTIME-1) +
...
Q09DemGapSte(allCy,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
  ...
  ( 
      ...
      sum(TSTEAM,
        ...
        p09CapSte(allCy,TSTEAM,YTIME-1)
      ) +
  SQRT(SQR(
      VmDemTotSte(allCy,YTIME) -
      sum(TSTEAM,
...
Q09CostVarProdSte(allCy,TSTEAM,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
    ...
    ) / (SUM(STECH$sameas(STECH,TSTEAM),imPlantEffByType(allCy,STECH,"effHeat","%fBaseY%")) + 1e-6) + !!i09EffSteThrm(TSTEAM,YTIME) +
    ...
    (
      ...
      smFracElecPriChp /
      (SUM(TCHP$sameas(TCHP,TSTEAM),VmPriceElecInd(allCy,TCHP,YTIME)) + 1e-6)
    )$TCHP(TSTEAM);
...
Q09GapShareSte(allCy,TSTEAM,YTIME)$(TIME(YTIME)$runCy(allCy)) ..
    ...
    (p09CostProdSte(allCy,TSTEAM,YTIME-1) + 1e-6) ** (-2) /
    SUM(TSTEAM2,
      !!i04MatFacPlaAvailCap(allCy,TSTEAM2,YTIME) *
      (p09CostProdSte(allCy,TSTEAM2,YTIME-1) + 1e-6) ** (-2)
    );
...
Q09ScrapRatePremature(allCy,TSTEAM,YTIME)$(TIME(YTIME)$runCy(allCy))..
    V09ScrapRatePremature(allCy,TSTEAM,YTIME)
        =E=
    (p09CostVarProdSte(allCy,TSTEAM,YTIME-1) + 1e-6) ** (-2) /
    (
      (p09CostVarProdSte(allCy,TSTEAM,YTIME-1) + 1e-6) ** (-2) +
      (( 
        i09ScaleEndogScrap *
