...
VConsFinEneCountry <- readGDX('./blabla.gdx', "VConsFinEneCountry", field = 'l')
...
a <- readSource("MENA_EDS", subtype =  map[map[["OPEN.PROM"]] == "VConsFinEneCountry", "MENA.EDS"])
getRegions(a) <- sub("MOR", "MAR", getRegions(a)) # fix wrong region names in MENA
# choose years and regions that both models have
years <- intersect(getYears(a,as.integer=TRUE),getYears(VConsFinEneCountry,as.integer=TRUE))
regs <- intersect(getRegions(a),getRegions(VConsFinEneCountry))
...
write.report(VConsFinEneCountry[,years,],file="reporting.mif",model="OPEN-PROM",unit="Mtoe",scenario="BASE")
...
year <- Reduce(intersect, list(getYears(a,as.integer=TRUE),getYears(b,as.integer=TRUE),getYears(VConsFinEneCountry,as.integer=TRUE)))
...
VPriceElecIndResConsu <- readGDX('./blabla.gdx', "VPriceElecIndResConsu", field = 'l')
#choose Industrial consumer /i/
sets1 <- readSets("sets.gms", "iSet")
elec_prices_Industry <- VPriceElecIndResConsu[,,sets1[1,1]]
getNames(elec_prices_Industry) <- "Electricity prices Industrial"
#choose Residential consumer /r/
sets2 <- readSets("sets.gms", "rSet")
elec_prices_Residential <- VPriceElecIndResConsu[,,sets2[1,1]]
...
#add model MENA_EDS data (choosing the correct variable from MENA by use of the MENA-PROM mapping)
z <- readSource("MENA_EDS", subtype =  map[map[["OPEN.PROM"]] == "VPriceElecIndResConsu", "MENA.EDS"])

getRegions(z) <- sub("MOR", "MAR", getRegions(z)) # fix wrong region names in MENA
# choose years and regions that both models have
years <- intersect(getYears(z,as.integer=TRUE),getYears(VPriceElecIndResConsu,as.integer=TRUE))
regs <- intersect(getRegions(z),getRegions(VPriceElecIndResConsu))
#choose Industrial consumer /i/
...
year <- Reduce(intersect, list(getYears(z,as.integer=TRUE),getYears(x,as.integer=TRUE),getYears(VPriceElecIndResConsu,as.integer=TRUE)))
x <- x[, year,]
# write data in mif file
...
VDemFinEneTranspPerFuel <- readGDX('./blabla.gdx', "VDemFinEneTranspPerFuel", field = 'l')

# aggregate from PROM fuels to subsectors
VDemTr_subsectors <- toolAggregate(VDemFinEneTranspPerFuel[,,unique(map_subsectors$EF)],dim=3,rel=map_subsectors,from="EF",to="SBS")
getItems(VDemTr_subsectors, 3) <- paste0("Final Energy Transport ", getItems(VDemTr_subsectors, 3))

#add model MENA_EDS data (choosing the correct variable from MENA by use of the MENA-PROM mapping)
a <- readSource("MENA_EDS", subtype =  map[map[["OPEN.PROM"]] == "VDemFinEneTranspPerFuel", "MENA.EDS"])
getRegions(a) <- sub("MOR", "MAR", getRegions(a)) # fix wrong region names in MENA
# choose years and regions that both models have
...
transportation <- dimSums(VDemFinEneTranspPerFuel, dim = 3)
...
sets10 <- sets5 %>% filter(EF %in% getItems(VDemFinEneTranspPerFuel,3.2))

#Aggregate model OPEN-PROM by subsector and by energy form 
VDemTr_by_energy_form <- toolAggregate(VDemFinEneTranspPerFuel[,,as.character(unique(sets10$EF))],dim=3.2,rel=sets10,from="EF",to="EFA")
...
#missing fuels
#v <- getItems(VDemFinEneTranspPerFuel,3.2)
#v <- as.data.frame(v)
#l <- v %>% filter(!v %in% as.character(unique(sets10$EF)))

#VDemTr_missing <- VDemFinEneTranspPerFuel[,,as.character(unique(l$v))]
