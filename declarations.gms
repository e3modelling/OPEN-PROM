...
QLftPcScrRate(allCy,DSBS,EF,TEA,YTIME)	"Compute the lifetime of passenger cars" 	
QActivGoodsTransp(allCy,TRANSE,YTIME)	"Compute goods transport activity"
QGapActivNewTech(allCy,TRANSE,YTIME)	    "Compute the gap in transport activity"	
QConsSpecFuelTech(allCy,TRANSE,TTECH,TEA,EF,YTIME)	"Compute Specific Fuel Consumption"
QCostTranspMeanConsSize(allCy,TRANSE,RCon,TTECH,TEA,YTIME)	"Compute transportation cost per mean and consumer size in KEuro per vehicle"
...
QCostTranspMatFac(allCy,TRANSE,RCon,TTECH,TEA,YTIME)	        "Compute transportation cost including maturity factor"	
QConsEachTechTransp(allCy,TRANSE,TTECH,EF,TEA,YTIME)	        "Compute consumption of each technology in transport sectors"
QFinDemFuel(allCy,TRANSE,EF,YTIME)	                "Compute final energy demand in transport per fuel"
...
qConsElecInd(allCy,YTIME)                     "Compute Consumption of electricity in industrial sectors"
...
QConsFuelNoHeat(allCy,DSBS,EF,YTIME)                "Compute fuel consumption"
...
QCostAvgVarChpFuel(allCy,CHP,YTIME)              "Compute Average variable including fuel electricity production cost per CHP plant"
QConsElecIndTertNonSub(allCy,INDDOM,YTIME)	          "Compute non-substituable electricity consumption in Industry and Tertiary"
QConsSub(allCy,DSBS,EF,YTIME)	      "Equation for consumption of remaining substitutble equipment"	
...
qConsFinEneAll(YTIME)                     "Compute total final energy consumption in ALL countries"
...
*' *** Prices
QFuelPriSepCarbonWght(allCy,SBS,EF,YTIME)	"Compute fuel prices per subsector and fuel, separate carbon value in each sector"	
QAvgFuelPriSub(allCy,DSBS,YTIME)	    "Compute average fuel price per subsector" 	
QElecPriIndResConsu(allCy,ESET,YTIME)	"Compute electricity price in Industrial and Residential Consumers"	
*' *** Miscellaneous
...
VAvgFuelPriSub(allCy,DSBS,YTIME)           "Average fuel prices per subsector (k$2015/toe)"
...
VConsFuelNoHeat(allCy,DSBS,EF,YTIME)         "Consumption of fuels in each demand subsector, excluding heat from heatpumps (Mtoe)"
VFinDemFuel(allCy,TRANSE,EF,YTIME)          "Final energy demand in transport subsectors per fuel (Mtoe)"
VLftPcScrRate(allCy,DSBS,EF,TEA,YTIME) "Lifetime of technologies (years)"

*' *** Transport Variables
VActivGoodsTransp(allCy,TRANSE,YTIME)	    "Goods transport acitivity (Gtkm)"	
VGapActivNewTech(allCy,TRANSE,YTIME)	"Gap in transport activity to be filled by new technologies ()"
...
VConsSpecFuelTech(allCy,TRANSE,TTECH,TEA,EF,YTIME)	"Specific Fuel Consumption ()"
...
VCostTranspMeanConsSize(allCy,TRANSE,RCon,TTECH,TEA,YTIME)	"Transportation cost per mean and consumer size (Keuro/vehicle)"
VTranspCostPerVeh(allCy,TRANSE,RCon,TTECH,TEA,YTIME)	"Transportation cost per mean and consumer size (Keuro/vehicle)"
VCostTranspMatFac(allCy,TRANSE,RCon,TTECH,TEA,YTIME)	"Transportation cost including maturity factor (Keuro/vehicle)"	
...
VConsElecIndTertNonSub(allCy,DSBS,YTIME)          "Consumption of non-substituable electricity in Industry and Tertiary (Mtoe)"
vConsElecInd(allCy,YTIME)              "Total Consumption of Electricity in industrial sectors (Mtoe)"
...
VConsSub(allCy,DSBS,EF,YTIME)                   "Consumption of remaining substitutable equipment (Mtoe)"
...
VCostAvgVarChpFuel(allCy,CHP,YTIME)                     "Average variable including fuel electricity production cost per CHP plant (Euro/KWh)"

*' *** REST OF ENERGY BALANCE SECTORS VARIABLES
VPlantEffPlantType(allCy,PGALL,YTIME)	                "Plant efficiency per plant type (1)"
vConsFinEneAll(YTIME)                                "Total final energy Consumption in ALL COUNTRIES (Mtoe)"
...
*' *** Prices Variables
VElecPriIndResConsu(allCy,ESET,YTIME)	                "Electricity price to Industrial and Residential Consumers (Euro2005/KWh)"
VFuelPriSepCarbonWght(allCy,SBS,EF,YTIME)	                    "Fuel prices per subsector and fuel  mutliplied by weights (kdollars2005/toe)"	
