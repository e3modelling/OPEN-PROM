...
iEffValueInDollars(allCy,SBS,YTIME)	            "Efficiency value (USD$2015/toe)"
...
iScenarioPri(WEF,NAP,YTIME)	                    "Scenario prices (KUSD$2015/toe)"	
...
iHydrogenPri(allCy,SBS,YTIME)	                "Total Hydrogen Cost Per Sector (USD$2015/toe)"
...
iCarbValYrExog(allCy,ytime)	                    "Carbon value for each year when it is exogenous (USD$2015/tn CO2)"
...
iVarCostTech(allCy,SBS,EF,YTIME)	            "Variable Cost of technology ()"
                                                    !! For transport (kUSD$2015/vehicle)
                                                    !! For Industrial sectors, except Iron and Steel (USD$2015/toe-year)
                                                    !! For Iron and Steel  (USD$2015/tn-of-steel)
                                                    !! For Domestic sectors  (USD$2015/toe-year)
iFixOMCostTech(allCy,SBS,EF,YTIME)              "Fixed O&M cost of technology (USD$2015/toe-year)"                                   
                                                    !! Fixed O&M cost of technology for Transport (kUSD$2015/vehicle)
                                                    !! Fixed O&M cost of technology for Industrial sectors-except Iron and Steel (USD$2015/toe-year)"                                            
                                                    !! Fixed O&M cost of technology for Iron and Steel (USD$2015/tn-of-steel)"                                          
                                                    !! Fixed O&M cost of technology for Domestic sectors (USD$2015/toe-year)"
iUsfEneConvSubTech(allCy,SBS,EF,YTIME)          "Useful Energy Conversion Factor per subsector and technology (1)"
iInvCostChp(allCy,DSBS,CHP,YTIME)               "Capital Cost per CHP plant type (kUSD$2015/KW)"
iFixOMCostPerChp(allCy,DSBS,CHP,YTIME)          "Fixed O&M cost per CHP plant type (USD$2015/KW)"
iVarCostChp(allCy,DSBS,CHP,YTIME)               "Variable (other than fuel) cost per CHP Type (Gross USD$2015/KW)"
...
iCapCostTech(allCy,SBS,EF,YTIME)                "Capital Cost of technology (various)"
                                                    !! - For transport is expressed in kUSD$2015 per vehicle
                                                    !! - For Industrial sectors (except Iron and Steel) is expressed in kUSD$2015/toe-year
                                                    !! - For Iron and Steel is expressed in kUSD$2015/tn-of-steel
                                                    !! - For Domestic Sectors is expressed in kUSD$2015/toe-year
...
QCostTranspPerMeanConsSize(allCy,TRANSE,RCon,TTECH,YTIME)  "Compute transportation cost per mean and consumer size in KUSD$2015 per vehicle"
...
QCostTranspPerVeh(allCy,TRANSE,RCon,TTECH,YTIME)	       "Compute transportation cost per mean and consumer size in KUSD$2015 per vehicle"
...
vCostHourProdInvCCS(allCy,PGALL,HOUR,YTIME)                "Hourly production cost of technology accounting for CCS acceptance (USD$2015/KWh)"
VCostPowGenLngTechNoCp(allCy,PGALL,ESET,YTIME)	           "Long-term average power generation cost (USD$2015/kWh)"
...
VCostVarTechNotPGSCRN(allCy,PGALL,YTIME)                   "Variable cost of technology excluding PGSCRN (USD$2015/KWh)"
vCostPowGenAvgShrt(allCy,ESET,YTIME)                       "Short-term average power generation cost (USD$2015/kWh)"	
VCostPowGenAvgLng(allCy,ESET,YTIME)	                       "Long-term average power generation cost (USD$2015/kWh)"	
vCostPowGenLongIntPri(allCy,PGALL,ESET,YTIME)              "Long term power generation cost of technologies including international prices of main fuels (kUSD$2015/toe)"	 
VPriceElecIndResNoCliPol(allCy,ESET,YTIME)                 "Electricity price to Industrial and Residential Consumers (USD$2015/KWh)"	
VCostPowGenLonNoClimPol(allCy,ESET,YTIME)                  "Long-term average power generation cost  excluding climate policies (USD$2015/kWh)"	
VCostAvgPowGenLonNoClimPol(allCy,PGALL,ESET,YTIME)	       "Long-term average power generation cost excluding climate policies(USD$2015/kWh)" 	
vCostPowGenShortIntPri(allCy,PGALL,ESET,YTIME)             "Short term power generation cost of technologies including international Prices of main fuels (kUSD$2015/toe)"		
vCostPowGenLonMin(allCy,PGALL,YTIME)	                   "Long-term minimum power generation cost (USD$2015/kWh)"	
...
VCostHourProdInvDec(allCy,PGALL,HOUR,YTIME)                "Hourly production cost of technology (USD$2015/KWh)"
VProdEstElecCHP(allCy,YTIME)	                           "Estimate the electricity of CHP Plants (1)"	
VCostProdSpecTech(allCy,PGALL,YTIME)	                   "Production cost of technology (USD$2015/KWh)"
...
VCostVarTechElec(allCy,PGALL,YTIME)	                       "Variable cost of technology (USD$2015/KWh)"	
...
VCostProdTeCHPreReplac(allCy,PGALL,YTIME)                  "Production cost of technology used in premature replacement (USD$2015/KWh)"
...
VCostVarTech(allCy,PGALL,YTIME)	                           "Variable cost of technology (USD$2015/KWh)"	
...
VCostHourProdInvDecNoCCS(allCy,PGALL,HOUR,YTIME)           "Hourly production cost of technology (USD$2015/KWh)"	
VCostProdTeCHPreReplacAvail(allCy,PGALL,PGALL2,YTIME)	   "Production cost of technology used in premature replacement including plant availability rate (USD$2015/KWh)"		
...
VCostTranspPerMeanConsSize(allCy,TRANSE,RCon,TTECH,YTIME)  "Transportation cost per mean and consumer size (KUSD$2015/vehicle)"
VCostTranspPerVeh(allCy,TRANSE,RCon,TTECH,YTIME)	       "Transportation cost per mean and consumer size (KUSD$2015/vehicle)"
VCostTranspMatFac(allCy,TRANSE,RCon,TTECH,YTIME)	       "Transportation cost including maturity factor (KUSD$2015/vehicle)"	
...
VCostElecProdCHP(allCy,DSBS,CHP,YTIME)                     "Electricity production cost per CHP plant and demand sector (USD$2015/KWh)"
VCostTech(allCy,DSBS,rCon,EF,YTIME)                        "Technology cost (KUSD$2015/toe)"
VCostTechIntrm(allCy,DSBS,rcon,EF,YTIME)                   "Intermediate technology cost (KUSD$2015/toe)"
VCostTechMatFac(allCy,DSBS,rCon,EF,YTIME)                  "Technology cost including maturity factor (KUSD$2015/toe)"
...
VCostProdCHPDem(allCy,DSBS,CHP,YTIME)                      "Variable including fuel electricity production cost per CHP plant and demand sector (USD$2015/KWh)"
VCostElcAvgProdCHP(allCy,CHP,YTIME)                        "Average Electricity production cost per CHP plant (USD$2015/KWh)"
VCostVarAvgElecProd(allCy,CHP,YTIME)                       "Average variable including fuel electricity production cost per CHP plant (USD$2015/KWh)"
...
*' *** Prices Variables
VPriceElecIndResConsu(allCy,ESET,YTIME)	                   "Electricity price to Industrial and Residential Consumers (USD$2015/KWh)"
VPriceFuelSepCarbonWght(allCy,SBS,EF,YTIME)	               "Fuel prices per subsector and fuel  mutliplied by weights (kUSD$2015/toe)"	
VAvgPowerGenLongTrm(allCy,ESET,YTIME)	                   "Long-term average power generation cost (USD$2015/kWh)"	 
VFuelPriSubNoCarb(allCy,SBS,EF,YTIME)	                   "Fuel prices per subsector and fuel  without carbon value (kUSD$2015/toe)"	
...
VCstCO2SeqCsts(allCy,YTIME)	                               "Cost curve for CO2 sequestration costs (USD$2015/tn of CO2 sequestrated)"
...
Positive Variables
VCarVal(allCy,NAP,YTIME)                                   "Carbon prices for all countries (USD$2015/tn CO2)"
VRenValue(YTIME)                                           "Renewable value (USD$2015/KWh)"
;
