...
QDemElec(allCy,YTIME)                      "Compute total electricity demand"
QConsElecFinDemSec(allCy,DSBS,YTIME)             "Compute electricity consumption per final demand sector"
QLoadBaseEstSec(allCy,YTIME)	               "Compute estimated base load"	
QLoadFacDom(allCy,YTIME)	               "Compute load factor of entire domestic system"
QLoadPeakElec(allCy,YTIME)	               "Compute elerctricity peak load"		
QLoadBaseMax(allCy,YTIME) 	               "Compute baseload corresponding to maximum load"
QLoadBaseElecExp(allCy,YTIME)	               "Compute electricity base load"
QShareNewTechCcs(allCy,PGALL,YTIME)	               "Compute SHRCAP"	
QCapGenPlannedPlant(allCy,PGALL,YTIME)	           "Compute electricity generation capacity"
...
qPotResCurr(allCy,PGRENEF,YTIME)	       "Compute current renewable potential" 	
...
QProdElecPowPlantsCy(allCy,PGALL,YTIME)   "Compute electricity production from power generation plants"	
QProdReqElec(allCy,YTIME)	               "Compute total required electricity production" 	
QGenEstElecChpPlants(allCy,YTIME)	               "Estimate the electricity of CHP Plants"	
QCapOverall(allCy,PGALL,YTIME)	           "Compute overall capacity"
QProdNonChp(allCy,YTIME)	           "Compute non CHP electricity production" 	
QCapAvgFacRes(allCy,PGALL,YTIME)	       "Compute the average capacity factor of RES"	
QCapGenNewResPlant(allCy,PGALL,YTIME)	       "Compute the new capacity added every year"	
...
QCostVarPlantTech(allCy,PGALL,YTIME)	   "Compute variable cost of technology" 
QElecPeakLoads(allCy,YTIME)	               "Compute Electricity peak loads"	
QCapGenPlant(allCy,PGALL,YTIME)	       "Compute electricity generation capacity"	
QShaNewEqPowPla(allCy,PGALL,YTIME)	   "Compute the power plant share in new equipment"	
QDecisNewInv(allCy,YTIME)	               "Compute for Power Plant new investment decision"		
QRenPotMaxAllow(allCy,PGRENEF,YTIME)   "Compute maximum allowed renewable potential"
QRenPotMinAllow(allCy,PGRENEF,YTIME)	   "Compute minimum allowed renewable potential" 
QRenTechMatMult(allCy,PGALL,YTIME)	       "Compute renewable technologies maturity multiplier"	
QGenEstElecChpPlants(allCy,CHP,YTIME)	       "Compute electricity production from CHP plants" 		 	
qScalWeibull(allCy,PGALL,HOUR,YTIME)       "Compute temporary variable facilitating the scaling in Weibull equation"	
QCapGenElecNoChp(allCy,YTIME)	               "Compute total electricity generation capacity excluding CHP plants"
QPotRenSuppCurve(allCy,PGRENEF,YTIME)	   "Compute renewable potential supply curve"		
QIndexEndogScrap(allCy,PGALL,YTIME)	       "Compute endogenous scrapping index" 	
...
QProdReqTotElec(allCy,YTIME)               "Compute total required electricity production"
QCapGenTotEstElec(allCy,YTIME)             "Compute Estimated total electricity generation capacity"	
QCapGenTotElec(allCy,YTIME)	               "Compute total electricity generation capacity"
...
QCostTranspPerMeanConsSize(allCy,TRANSE,RCon,TTECH,YTIME)	"Compute transportation cost per mean and consumer size in KEuro per vehicle"
QSortTechVarCost(allCy,TRANSE,Rcon,YTIME)	                    "Compute technology sorting based on variable cost"	
QShareTechSectoral(allCy,TRANSE,EF,YTIME)	            "Compute technology sorting based on variable cost and new equipment"	
QCostTranspPerVeh(allCy,TRANSE,RCon,TTECH,YTIME)	        "Compute transportation cost per mean and consumer size in KEuro per vehicle"
...
QExprtsFakeEneBrnch(allCy,EFS,YTIME)	                 "Compute fake exports"	
QImptsFakeEneBrnch(allCy,EFS,YTIME)	             "Compute fake imports"	
...
VLoadBaseEstSec(allCy,YTIME)	              "Estimated base load (GW)"	
VDemElec(allCy,YTIME)                     "Total electricity demand (TWh)"
VLoadFacDom(allCy,YTIME)                  "Electricity load factor for entire domestic system"	
VLoadPeakElec(allCy,YTIME)	              "Electricity peak load (GW)"	
VLoadBaseMax(allCy,YTIME)	              "Baseload corresponding to Maximum Load Factor (1)"
...
vPotResCurr(allCy,PGRENEF,YTIME)	       "Current renewable potential (GW)"			
VProdReqElec(allCy,YTIME)	               "Total required electricity production (TWh)"	
VCapOverall(allCy,PGALL,YTIME)	           "Overall Capacity (MW)"	
VCostVarTechNotPGSCRN(allCy,PGALL,YTIME)  "Variable cost of technology excluding PGSCRN (Euro/KWh)"
...
vCostPowGenLongIntPri(allCy,PGALL,ESET,YTIME) "Long term power generation cost of technologies including international prices of main fuels (kEuro05/toe)"
VRenShareElecProdSub(allCy,YTIME)	      "Renewables share in gross electricity production for subsdidized renewables (1)"	 
VPriceElecIndResNoCliPol(allCy,ESET,YTIME)  "Electricity price to Industrial and Residential Consumers (Euro2005/KWh)"	
VCostPowGenLonNoClimPol(allCy,ESET,YTIME) "Long-term average power generation cost  excluding climate policies (Euro2005/kWh)"	
VCostAvgPowGenLonNoClimPol(allCy,PGALL,ESET,YTIME)	"Long-term average power generation cost excluding climate policies(Euro2005/kWh)" 	
vCostPowGenShortIntPri(allCy,PGALL,ESET,YTIME)  "Short term power generation cost of technologies including international Prices of main fuels (kEuro05/toe)"		
vCostPowGenLonMin(allCy,PGALL,YTIME)	  "Long-term minimum power generation cost (Euro2005/kWh)"	
VGenEstElecChpPlants(allCy,CHP,YTIME)	          "CHP electricity production (TWh)"	
...
VProdElecPowPlantsCy(allCy,PGALL,YTIME)  "Electricity production (TWh)"	
VCapAvgFacRes(allCy,PGALL,YTIME)	      "The average capacity factor of RES (1)"
VCapGenNewResPlant(allCy,PGALL,YTIME)	      "The new capacity added every year (MW)"	
VProdNonChp(allCy,YTIME)	          "Non CHP total electricity production (TWh)"				
VCostHourProdInvDec(allCy,PGALL,HOUR,YTIME)     "Hourly production cost of technology (Euro/KWh)"
VGenEstElecChpPlants(allCy,YTIME)	              "Estimate the electricity of CHP Plants (1)"	
...
VCostVarPlantTech(allCy,PGALL,YTIME)	  "Variable cost of technology (Euro/KWh)"	
VDecisNewInv(allCy,YTIME)	              "Power plant sorting for new investment decision according to total cost (1)"	
VShaNewEqPowPla(allCy,PGALL,YTIME)	  "Power plant share in new equipment (1)"	
...
VRenPotMaxAllow(allCy,PGRENEF,YTIME)  "Maximum allowed renewable potential (GW)"
VRenPotMinAllow(allCy,PGRENEF,YTIME)	  "Minimum allowed renewable potential (GW)"		
vScalWeibull(allCy,PGALL,HOUR,YTIME)      "Temporary variable facilitating the scaling in Weibull equation"	
VCapGenPlant(allCy,PGALL,YTIME)	  "Electricity generation plants capacity (GW)"	
VGapGenCapPowerDiff(allCy,YTIME)	          "Gap in total generation capacity to be filled by new equipment (GW)"		
VCostProdTechPreReplac(allCy,PGALL,YTIME) "Production cost of technology used in premature replacement (Euro/KWh)"
VIndexEndogScrap(allCy,PGALL,YTIME)	      "Index used for endogenous power plants scrapping (1)"			
VSensCcs(allCy,YTIME)	                  "Variable that controlls the sensitivity of CCS acceptance (1)"			
VLoadBaseElecExp(allCy,YTIME)	              "Corrected base load (GW)"
VCapGenPlannedPlant(allCy,PGALL,YTIME)	      "Electricity generation plants capacity (GW)"	
VCapGenElecNoChp(allCy,YTIME)	              "Total electricity generation capacity excluding CHP (GW)"	
VCostVarTech(allCy,PGALL,YTIME)	          "Variable cost of technology (Euro/KWh)"	
...
VCostTranspPerMeanConsSize(allCy,TRANSE,RCon,TTECH,YTIME)	"Transportation cost per mean and consumer size (Keuro/vehicle)"
VCostTranspPerVeh(allCy,TRANSE,RCon,TTECH,YTIME)	"Transportation cost per mean and consumer size (Keuro/vehicle)"
VCostTranspMatFac(allCy,TRANSE,RCon,TTECH,YTIME)	"Transportation cost including maturity factor (Keuro/vehicle)"	
VSortTechVarCost(allCy,TRANSE,Rcon,YTIME)	            "Technology sorting based on variable cost (1)"	
...
VCostTechIntrm(allCy,DSBS,rcon,EF,YTIME)            "Intermediate technology cost (Keuro/toe)"
VCostTechMatFac(allCy,DSBS,rCon,EF,YTIME)             "Technology cost including maturity factor (Keuro/toe)"
