...
*'
*' These sets index the emulator tables loaded (via -Idir=./data) from:
*'   iBmswasSupplyCoef_<source>.csv    (supply curve a,b,c; COEF)
*'   iBmswasLandEmisCoef_<source>.csv  (land CO2 curve ea,eb,ec; EMTYPE x ECOEF)
*'   iBmswasAgriEmis_<source>.csv      (agriculture CH4/N2O direct values; EMTYPE, Q-independent)
*' where <source> = %landUseEmulator% (globiom or magpie); generated by mrprom
*' calcBmswasSupplyCoefGLOBIOM / calcBmswasLandEmisCoefGLOBIOM / calcBmswasAgriEmisGLOBIOM
*' (via fullOPEN-PROM). Active when %bmswasPriceMode% == curve.
*' ============================================================
...
*' fitted by NLLS to the GLOBIOM (price, quantity) data points:
*'   a = intercept / minimum price (k$2015/toe)
*'   b = slope scaling factor
*'   c = curvature exponent (constrained to [0.1, 5.0] in the fit)
COEF "Coefficients for GLOBIOM power-law biomass supply curve (P = a + b*Q^c)"
/ a, b, c /

*' Emission species tracked for BMSWAS land-use change impacts from GLOBIOM:
...
*'   N2OLandUse = N2O from soil disturbance (MtN2O/yr)
...
ALIAS (YYTIME2,ytime);
ALIAS (PGEF2,PGEF);
* DSBS2 is a dummy alias of DSBS used as an inner summation index in the GLOBIOM
* BMSWAS supply curve equation (Q08PriceFuelSubsecCarVal, module 08 equations.gms).
* A separate alias is needed because DSBS is already used as an outer equation index.
ALIAS (DSBS2,DSBS);
