...
qDummyObj(allCy,YTIME)$(TIME(YTIME) and runCy(allCy)).. 
  ...
  SUM((DSBS,EFS)$(sameas("SE", DSBS) or sameas("HOU",DSBS) or (sameas("AG",DSBS) and EU28(allCy)) and SECtoEF(DSBS,EFS)),
    SQR(
  ...
  SUM((DSBS,EFS)$(INDSE(DSBS) or sameas("NEN",DSBS) or sameas("PCH",DSBS) and SECtoEF(DSBS,EFS)),
    SQR(
