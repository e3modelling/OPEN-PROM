...
qDummyObjPGALL(allCy,YTIME)$(TIME(YTIME) and runCy(allCy))..
  ...
  SUM(PGALL,
    SQR(
      VmProdElec(allCy,PGALL,YTIME) / SUM(PGALL2,VmProdElec(allCy,PGALL2,YTIME)) - 
      t04SharePowPlaNewEq(allCy,PGALL,YTIME)
    ) +
    0.001 * SQR(
      i04MatFacPlaAvailCap(allCy,PGALL,YTIME) / i04MatFacPlaAvailCap(allCy,PGALL,YTIME-1) - 1
    ) +
    0.001 * SQR(
      i04ScaleEndogScrap(allCy,PGALL,YTIME) / i04ScaleEndogScrap(allCy,PGALL,YTIME-1) - 1
    )
  ) / CARD(PGALL) 
  ;
