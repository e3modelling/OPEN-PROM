...
QCurrRenPot(allCy,PGRENEF,YTIME)$(TIME(YTIME)$(runCy(allCy)))..

         VCurrRenPot(allCy,PGRENEF,YTIME) 
         =E=
         ( VMaxmAllowRenPotent(allCy,PGRENEF,YTIME) + iMinRenPotential(allCy,PGRENEF,YTIME))/2;
...
QChpElecPlants(allCy,CHP,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VElecCapChpPla(allCy,CHP,YTIME)
         =E=
         1/sTWhToMtoe * sum(INDDOM,VConsFuel(allCy,INDDOM,CHP,YTIME)) * VElecIndPrices(allCy,YTIME)/
...
QLambda(allCy,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         (1 - exp( -VLoadCurveConstr(allCy,YTIME)*sGwToTwhPerYear))  / (0.0001+VLoadCurveConstr(allCy,YTIME))
             =E=
         (VElecDem(allCy,YTIME) - sGwToTwhPerYear*VCorrBaseLoad(allCy,YTIME))
         / (VElecPeakLoad(allCy,YTIME) - VCorrBaseLoad(allCy,YTIME));
...
QElecDem(allCy,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VElecDem(allCy,YTIME)
             =E=
         1/sTWhToMtoe *
         ( VFeCons(allCy,"ELC",YTIME) + VFNonEnCons(allCy,"ELC",YTIME) + VLosses(allCy,"ELC",YTIME)
           + VEnCons(allCy,"ELC",YTIME) - iNetImp(allCy,"ELC",YTIME)
         );
...
QEstBaseLoad(allCy,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VEstBaseLoad(allCy,YTIME)
             =E=
         (
             sum(DSBS, iBaseLoadShareDem(allCy,DSBS,YTIME)*VElecConsAll(allCy,DSBS,YTIME))*(1+iRateLossesFinCons(allCy,"ELC",YTIME))*
             (1 - VNetImports(allCy,"ELC",YTIME)/(sum(DSBS, VElecConsAll(allCy,DSBS,YTIME))+VLosses(allCy,"ELC",YTIME)))
             + 0.5*VEnCons(allCy,"ELC",YTIME)
...
QLoadFacDom(allCy,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VLoadFacDom(allCy,YTIME)
             =E=
         (sum(INDDOM,VConsFuel(allCy,INDDOM,"ELC",YTIME)) + sum(TRANSE, VDemTr(allCy,TRANSE,"ELC",YTIME)))/
         (sum(INDDOM,VConsFuel(allCy,INDDOM,"ELC",YTIME)/iLoadFacElecDem(INDDOM)) + 
         sum(TRANSE, VDemTr(allCy,TRANSE,"ELC",YTIME)/iLoadFacElecDem(TRANSE)));         
...
*' load factor.
QElecPeakLoad(allCy,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VElecPeakLoad(allCy,YTIME)
             =E=
         VElecDem(allCy,YTIME)/(VLoadFacDom(allCy,YTIME)*sGwToTwhPerYear);
...
QBslMaxmLoad(allCy,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         (VElecDem(allCy,YTIME)-VBslMaxmLoad(allCy,YTIME)*sGwToTwhPerYear)
             =E=
         iMxmLoadFacElecDem(allCy,YTIME)*(VElecPeakLoad(allCy,YTIME)-VBslMaxmLoad(allCy,YTIME))*sGwToTwhPerYear;  
...
QElecBaseLoad(allCy,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VCorrBaseLoad(allCy,YTIME)
             =E=
         (1/(1+exp(iBslCorrection(allCy,YTIME)*(VEstBaseLoad(allCy,YTIME)-VBslMaxmLoad(allCy,YTIME)))))*VEstBaseLoad(allCy,YTIME)
        +(1-1/(1+exp(iBslCorrection(allCy,YTIME)*(VEstBaseLoad(allCy,YTIME)-VBslMaxmLoad(allCy,YTIME)))))*VBslMaxmLoad(allCy,YTIME);
...
QTotReqElecProd(allCy,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VTotReqElecProd(allCy,YTIME)
             =E=
         sum(HOUR, (VElecPeakLoad(allCy,YTIME)-VCorrBaseLoad(allCy,YTIME))
                   * exp(-VLoadCurveConstr(allCy,YTIME)*(0.25+(ord(HOUR)-1)))
             ) + 9*VCorrBaseLoad(allCy,YTIME);   
...
QTotEstElecGenCap(allCy,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
        VTotElecGenCapEst(allCy,YTIME)
             =E=
        VTotElecGenCap(allCy,YTIME-1) * VElecPeakLoad(allCy,YTIME)/VElecPeakLoad(allCy,YTIME-1);          
...
QTotElecGenCap(allCy,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
        VTotElecGenCap(allCy,YTIME) 
        =E=
     VTotElecGenCapEst(allCy,YTIME);  
...
QHourProdCostInv(allCy,PGALL,HOUR,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VHourProdCostTech(allCy,PGALL,HOUR,YTIME)
                  =E=
                    ...
                    + sum(PGEF$PGALLtoEF(PGALL,PGEF), (VFuelPriceSub(allCy,"PG",PGEF,YTIME)+
                        iCO2CaptRate(allCy,PGALL,YTIME)*VCO2SeqCsts(allCy,YTIME)*1e-3*
...
QHourProdCostInvDec(allCy,PGALL,HOUR,YTIME)$(TIME(YTIME) $NOCCS(PGALL) $runCy(allCy)) ..
         VHourProdCostTechNoCCS(allCy,PGALL,HOUR,YTIME) =E=
         VPowerPlantNewEq(allCy,PGALL,YTIME)*VHourProdCostTech(allCy,PGALL,HOUR,YTIME)+
         sum(CCS$CCS_NOCCS(CCS,PGALL), VPowerPlaShrNewEq(allCy,CCS,YTIME)*VHourProdCostTech(allCy,CCS,HOUR,YTIME)); 
...
QGammaInCcsDecTree(allCy,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
...
qHourProdCostInvDecisionsAfterCCS(allCy,PGALL,HOUR,YTIME)$(TIME(YTIME) $(CCS(PGALL) or NOCCS(PGALL)) $runCy(allCy)) ..
         vHourProdCostTechAfterCCS(allCy,PGALL,HOUR,YTIME) 
         =E=
          VHourProdCostTech(allCy,PGALL,HOUR,YTIME)**(-VSensCcs(allCy,YTIME));
...
QProdCostInvDecis(allCy,PGALL,YTIME)$(TIME(YTIME) $(CCS(PGALL) or NOCCS(PGALL)) $runCy(allCy)) ..
         VProdCostTechnology(allCy,PGALL,YTIME) 
         =E=  
         sum(HOUR,VHourProdCostTech(allCy,PGALL,HOUR,YTIME)**(-VSensCcs(allCy,YTIME))) ;
...
QShrcap(allCy,PGALL,YTIME)$(TIME(YTIME) $CCS(PGALL) $runCy(allCy))..
         VPowerPlaShrNewEq(allCy,PGALL,YTIME) =E=
         1.1 *VProdCostTechnology(allCy,PGALL,YTIME)
         /(1.1*VProdCostTechnology(allCy,PGALL,YTIME)
           + sum(PGALL2$CCS_NOCCS(PGALL,PGALL2),VProdCostTechnology(allCy,PGALL2,YTIME))
           );         
...
QShrcapNoCcs(allCy,PGALL,YTIME)$(TIME(YTIME) $NOCCS(PGALL) $runCy(allCy))..
         VPowerPlantNewEq(allCy,PGALL,YTIME) 
         =E= 
         1 - sum(CCS$CCS_NOCCS(CCS,PGALL), VPowerPlaShrNewEq(allCy,CCS,YTIME));
...
QVarCostTech(allCy,PGALL,YTIME)$(time(YTIME) $runCy(allCy))..
         VVarCostTech(allCy,PGALL,YTIME) 
             =E=
         (iVarCost(PGALL,YTIME)/1E3 + sum(PGEF$PGALLtoEF(PGALL,PGEF), (VFuelPriceSub(allCy,"PG",PGEF,YTIME)/1.2441+
         iCO2CaptRate(allCy,PGALL,YTIME)*VCO2SeqCsts(allCy,YTIME)*1e-3*iCo2EmiFac(allCy,"PG",PGEF,YTIME)
         + (1-iCO2CaptRate(allCy,PGALL,YTIME))*1e-3*iCo2EmiFac(allCy,"PG",PGEF,YTIME)
          *(sum(NAP$NAPtoALLSBS(NAP,"PG"),VCarVal(allCy,NAP,YTIME))))
          *sTWhToMtoe/iPlantEffByType(allCy,PGALL,YTIME))$(not PGREN(PGALL)));
...
QVarCostTechNotPGSCRN(allCy,PGALL,YTIME)$(time(YTIME) $(not PGSCRN(PGALL)) $runCy(allCy))..
         VVarCostTechNotPGSCRN(allCy,PGALL,YTIME) 
              =E=
          VVarCostTech(allCy,PGALL,YTIME)**(-5);
...
QProdCostTechPreReplac(allCy,PGALL,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VProdCostTechPreReplac(allCy,PGALL,YTIME) =e=
                        (
                          ((iDisc(allCy,"PG",YTIME) * exp(iDisc(allCy,"PG",YTIME)*iTechLftPlaType(allCy,PGALL))/
                          (exp(iDisc(allCy,"PG",YTIME)*iTechLftPlaType(allCy,PGALL)) -1))
                           ...
                           (VFuelPriceSub(allCy,"PG",PGEF,YTIME)+
                            iCO2CaptRate(allCy,PGALL,YTIME)*VCO2SeqCsts(allCy,YTIME)*1e-3*iCo2EmiFac(allCy,"PG",PGEF,YTIME) +
                             (1-iCO2CaptRate(allCy,PGALL,YTIME))*1e-3*iCo2EmiFac(allCy,"PG",PGEF,YTIME)*
                         (sum(NAP$NAPtoALLSBS(NAP,"PG"),VCarVal(allCy,NAP,YTIME))))
                                 *sTWhToMtoe/iPlantEffByType(allCy,PGALL,YTIME))$(not PGREN(PGALL)))
...
QProdCostTechPreReplacAvail(allCy,PGALL,PGALL2,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VProdCostTechPreReplacAvail(allCy,PGALL,PGALL2,YTIME) =E=
         iAvailRate(PGALL,YTIME)/iAvailRate(PGALL2,YTIME)*VProdCostTechPreReplac(allCy,PGALL,YTIME)+
         VVarCostTech(allCy,PGALL,YTIME)*(1-iAvailRate(PGALL,YTIME)/iAvailRate(PGALL2,YTIME));  
...
QEndogScrapIndex(allCy,PGALL,YTIME)$(TIME(YTIME) $(not PGSCRN(PGALL)) $runCy(allCy))..
         VEndogScrapIndex(allCy,PGALL,YTIME)
                 =E=
         VVarCostTechNotPGSCRN(allCy,PGALL,YTIME)/
         (VVarCostTechNotPGSCRN(allCy,PGALL,YTIME)+(iScaleEndogScrap(PGALL)*
         sum(PGALL2,VProdCostTechPreReplacAvail(allCy,PGALL,PGALL2,YTIME)))**(-5));
...
QElecGenNoChp(allCy,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VElecGenNoChp(allCy,YTIME)
          =E=
VTotElecGenCap(allCy,YTIME) - SUM(CHP,VElecCapChpPla(allCy,CHP,YTIME)*0.85);      
...
QGapPowerGenCap(allCy,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VGapPowerGenCap(allCy,YTIME)
             =E=
 (        (  VElecGenNoChp(allCy,YTIME) - VElecGenNoChp(allCy,YTIME-1) + sum(PGALL,VElecGenPlanCap(allCy,PGALL,YTIME-1) * 
 (1 - VEndogScrapIndex(allCy,PGALL,YTIME))) +
          sum(PGALL, (iPlantDecomSched(allCy,PGALL,YTIME)-iDecInvPlantSched(allCy,PGALL,YTIME))*iAvailRate(PGALL,YTIME))
          + Sum(PGALL$PGSCRN(PGALL), (VElecGenPlantsCapac(allCy,PGALL,YTIME-1)-iPlantDecomSched(allCy,PGALL,YTIME))/
          iTechLftPlaType(allCy,PGALL))
       )
  + 0 + SQRT( SQR(       (  VElecGenNoChp(allCy,YTIME) - VElecGenNoChp(allCy,YTIME-1) +
        sum(PGALL,VElecGenPlanCap(allCy,PGALL,YTIME-1) * (1 - VEndogScrapIndex(allCy,PGALL,YTIME))) +
          sum(PGALL, (iPlantDecomSched(allCy,PGALL,YTIME)-iDecInvPlantSched(allCy,PGALL,YTIME))*iAvailRate(PGALL,YTIME))
          + Sum(PGALL$PGSCRN(PGALL), (VElecGenPlantsCapac(allCy,PGALL,YTIME-1)-iPlantDecomSched(allCy,PGALL,YTIME))/
          iTechLftPlaType(allCy,PGALL))
       ) -0) + SQR(1e-10) ) )/2;
...
qScalWeibull(allCy,PGALL,HOUR,YTIME)$((not CCS(PGALL))$TIME(YTIME) $runCy(allCy))..
         ...
         =E=
         (VHourProdCostTech(allCy,PGALL,HOUR,YTIME)$(not NOCCS(PGALL))
         +
          VHourProdCostTechNoCCS(allCy,PGALL,HOUR,YTIME)$NOCCS(PGALL))**(-6);     
...
*' the specified range.
QRenPotSupplyCurve(allCy,PGRENEF,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VRenPotSupplyCurve(allCy,PGRENEF,YTIME) =E=
...
QMaxmAllowRenPotent(allCy,PGRENEF,YTIME)$(TIME(YTIME)$(runCy(allCy)))..      
         VMaxmAllowRenPotent(allCy,PGRENEF,YTIME) =E=
         ( VRenPotSupplyCurve(allCy,PGRENEF,YTIME)+ iMaxRenPotential(allCy,PGRENEF,YTIME))/2;
...
qMnmAllowRenPot(allCy,PGRENEF,YTIME)$(TIME(YTIME)$(runCy(allCy)))..  
         vMnmAllowRenPot(allCy,PGRENEF,YTIME) =E=
         ( VRenPotSupplyCurve(allCy,PGRENEF,YTIME) + iMinRenPotential(allCy,PGRENEF,YTIME))/2;
...
QRenTechMatMult(allCy,PGALL,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         ...
         (
           1/(1+exp(5*(
                 ...
                 VElecGenPlanCap(allCy,PGALL2,YTIME-1))/VCurrRenPot(allCy,PGRENEF,YTIME))-0.6)))
           )$PGREN(PGALL);  
...
QScalWeibullSum(allCy,PGALL,YTIME)$((not CCS(PGALL)) $TIME(YTIME) $runCy(allCy))..
         ...
         =E=
              ...
              sum(HOUR,
                 (VHourProdCostTech(allCy,PGALL,HOUR,YTIME)$(not NOCCS(PGALL))
                 +
                 VHourProdCostTechNoCCS(allCy,PGALL,HOUR,YTIME)$NOCCS(PGALL)
                 )**(-6)
              ); 
...
QNewInvDecis(allCy,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VNewInvDecis(allCy,YTIME)
             =E=
...
QPowPlaShaNewEquip(allCy,PGALL,YTIME)$(TIME(YTIME) $runCy(allCy)) ..
        VPowPlaShaNewEquip(allCy,PGALL,YTIME)
             =E=
         ( VScalWeibullSum(allCy,PGALL,YTIME)/ VNewInvDecis(allCy,YTIME))$(not CCS(PGALL))
          +
          sum(NOCCS$CCS_NOCCS(PGALL,NOCCS),VPowPlaShaNewEquip(allCy,NOCCS,YTIME))$CCS(PGALL);
...
QElecGenCapacity(allCy,PGALL,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VElecGenPlantsCapac(allCy,PGALL,YTIME)
             =E=
         (VElecGenPlanCap(allCy,PGALL,YTIME-1)*VEndogScrapIndex(allCy,PGALL,YTIME-1)
          +(VPowPlaShaNewEquip(allCy,PGALL,YTIME) * VGapPowerGenCap(allCy,YTIME))$( (not CCS(PGALL)) AND (not NOCCS(PGALL)))
          +(VPowPlaShaNewEquip(allCy,PGALL,YTIME) * VPowerPlantNewEq(allCy,PGALL,YTIME) * VGapPowerGenCap(allCy,YTIME))$NOCCS(PGALL)
          +(VPowPlaShaNewEquip(allCy,PGALL,YTIME) * VPowerPlantNewEq(allCy,PGALL,YTIME) * VGapPowerGenCap(allCy,YTIME))$CCS(PGALL)
          + iDecInvPlantSched(allCy,PGALL,YTIME) * iAvailRate(PGALL,YTIME)
          - iPlantDecomSched(allCy,PGALL,YTIME) * iAvailRate(PGALL,YTIME)
         )
         - ((VElecGenPlantsCapac(allCy,PGALL,YTIME-1)-iPlantDecomSched(allCy,PGALL,YTIME-1))* 
...
QElecGenCap(allCy,PGALL,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VElecGenPlanCap(allCy,PGALL,YTIME)
             =E=
         ( VElecGenPlantsCapac(allCy,PGALL,YTIME) + 1e-6 + SQRT( SQR(VElecGenPlantsCapac(allCy,PGALL,YTIME)-1e-6) + SQR(1e-4) ) )/2;

*' Compute the variable cost of each power plant technology for every region,
*' by utilizing the maturity factor related to plant dispatching.
QVarCostTechnology(allCy,PGALL,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VVarCostTechnology(allCy,PGALL,YTIME)
         =E=  
          iMatureFacPlaDisp(allCy,PGALL,YTIME)*VVarCostTech(allCy,PGALL,YTIME)**(-2);
...
QElecPeakLoads(allCy,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         ...
         =E= 
         sum(PGALL, VVarCostTechnology(allCy,PGALL,YTIME));     
...
QElectrPeakLoad(allCy,PGALL,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VPowPlantSorting(allCy,PGALL,YTIME)
                 =E=
         VVarCostTechnology(allCy,PGALL,YTIME)
         /
         VElecPeakLoads(allCy,YTIME);  
...
QNewCapYearly(allCy,PGALL,YTIME)$(PGREN(PGALL)$TIME(YTIME)$runCy(allCy))..
         VNewCapYearly(allCy,PGALL,YTIME) =e=
VElecGenPlanCap(allCy,PGALL,YTIME)- VElecGenPlanCap(allCy,PGALL,YTIME-1);                       
...
QAvgCapFacRes(allCy,PGALL,YTIME)$(PGREN(PGALL)$TIME(YTIME)$runCy(allCy))..
   VAvgCapFacRes(allCy,PGALL,YTIME)
      =E=
    (iAvailRate(PGALL,YTIME)*VNewCapYearly(allCy,PGALL,YTIME)+
     iAvailRate(PGALL,YTIME-1)*VNewCapYearly(allCy,PGALL,YTIME-1)+
     iAvailRate(PGALL,YTIME-2)*VNewCapYearly(allCy,PGALL,YTIME-2)+
     iAvailRate(PGALL,YTIME-3)*VNewCapYearly(allCy,PGALL,YTIME-3)+
     iAvailRate(PGALL,YTIME-4)*VNewCapYearly(allCy,PGALL,YTIME-4)+
     iAvailRate(PGALL,YTIME-5)*VNewCapYearly(allCy,PGALL,YTIME-5)+
     iAvailRate(PGALL,YTIME-6)*VNewCapYearly(allCy,PGALL,YTIME-6)+
     iAvailRate(PGALL,YTIME-7)*VNewCapYearly(allCy,PGALL,YTIME-7))/
(VNewCapYearly(allCy,PGALL,YTIME)+VNewCapYearly(allCy,PGALL,YTIME-1)+VNewCapYearly(allCy,PGALL,YTIME-2)+
VNewCapYearly(allCy,PGALL,YTIME-3)+VNewCapYearly(allCy,PGALL,YTIME-4)+VNewCapYearly(allCy,PGALL,YTIME-5)+
VNewCapYearly(allCy,PGALL,YTIME-6)+VNewCapYearly(allCy,PGALL,YTIME-7));
...
QOverallCap(allCy,PGALL,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
     VOverallCap(allCy,PGALL,YTIME)
     =E=
VElecGenPlanCap(allCy,pgall,ytime)$ (not PGREN(PGALL))
+VAvgCapFacRes(allCy,PGALL,YTIME-1)*(VNewCapYearly(allCy,PGALL,YTIME)/iAvailRate(PGALL,YTIME)+
VOverallCap(allCy,PGALL,YTIME-1)
/VAvgCapFacRes(allCy,PGALL,YTIME-1))$PGREN(PGALL);
...
QScalFacPlantDispatch(allCy,HOUR,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         sum(PGALL,
                 (VOverallCap(allCy,PGALL,YTIME)+
                 sum(CHP$CHPtoEON(CHP,PGALL),VElecCapChpPla(allCy,CHP,YTIME)))*
                 exp(-VScalFacPlaDisp(allCy,HOUR,YTIME)/VPowPlantSorting(allCy,PGALL,YTIME))
                 )
         =E=
         (VElecPeakLoad(allCy,YTIME) - VCorrBaseLoad(allCy,YTIME))
         * exp(-VLoadCurveConstr(allCy,YTIME)*(0.25 + ord(HOUR)-1))
...
QElecChpPlants(allCy,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VElecChpPlants(allCy,YTIME) 
         =E=
         ( (1/0.086 * sum((INDDOM,CHP),VConsFuel(allCy,INDDOM,CHP,YTIME)) * VElecIndPrices(allCy,YTIME)) + 
         iMxmShareChpElec(allCy,YTIME)*VElecDem(allCy,YTIME) - SQRT( SQR((1/0.086 * sum((INDDOM,CHP),VConsFuel(allCy,INDDOM,CHP,YTIME)) * 
         VElecIndPrices(allCy,YTIME)) - 
         iMxmShareChpElec(allCy,YTIME)*VElecDem(allCy,YTIME)) + SQR(1E-4) ) )/2;
...
*' demand. 
QNonChpElecProd(allCy,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VNonChpElecProd(allCy,YTIME) 
         =E=
  (VElecDem(allCy,YTIME) - VElecChpPlants(allCy,YTIME));  
...
*' dispatching. 
QReqElecProd(allCy,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
VReqElecProd(allCy,YTIME) 
                   =E=
         sum(hour, sum(CHP,VElecCapChpPla(allCy,CHP,YTIME)*exp(-VScalFacPlaDisp(allCy,HOUR,YTIME)/ 
         sum(pgall$chptoeon(chp,pgall),VPowPlantSorting(allCy,PGALL,YTIME)))));
...
QElecProdPowGenPlants(allCy,PGALL,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VElecProd(allCy,PGALL,YTIME)
                 =E=
         VNonChpElecProd(allCy,YTIME) /
         (VTotReqElecProd(allCy,YTIME)- VReqElecProd(allCy,YTIME))
         * VElecGenPlanCap(allCy,PGALL,YTIME)* sum(HOUR, exp(-VScalFacPlaDisp(allCy,HOUR,YTIME)/VPowPlantSorting(allCy,PGALL,YTIME)));
...
QElecProdChpPlants(allCy,CHP,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VChpElecProd(allCy,CHP,YTIME)
                 =E=
        sum(INDDOM,VConsFuel(allCy,INDDOM,CHP,YTIME)) / SUM(chp2,sum(INDDOM,VConsFuel(allCy,INDDOM,CHP2,YTIME)))*
        (VElecDem(allCy,YTIME) - SUM(PGALL,VElecProd(allCy,PGALL,YTIME)));
...
qShareRenGrossElecProd(allCy,YTIME)$(TIME(YTIME)$(runCy(allCy)))..

                 vResShareGrossElecProd(allCy,YTIME) 
                 =E=
                 (SUM(PGNREN$((not sameas("PGASHYD",PGNREN)) $(not sameas("PGSHYD",PGNREN)) $(not sameas("PGLHYD",PGNREN)) ),
                         VElecProd(allCy,PGNREN,YTIME)))/
                 (SUM(PGALL,VElecProd(allCy,PGALL,YTIME))+ 
                 1e-3*sum(DSBS,sum(CHP$SECTTECH(DSBS,CHP),VConsFuel(allCy,DSBS,CHP,YTIME)))/8.6e-5*VElecIndPrices(allCy,YTIME) + 
                 1/0.086 *VNetImports(allCy,"ELC",YTIME));         
...
QLonPowGenCostTechNoCp(allCy,PGALL,ESET,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VLongPowGenCost(allCy,PGALL,ESET,YTIME)
                 =E=
             ...
             sum(PGEF$PGALLTOEF(PGALL,PGEF),
                 (iVarCost(PGALL,YTIME)/1000+(VFuelPriceSub(allCy,"PG",PGEF,YTIME)/1.2441+
                 iCO2CaptRate(allCy,PGALL,YTIME)*VCO2SeqCsts(allCy,YTIME)*1e-3*iCo2EmiFac(allCy,"PG",PGEF,YTIME) +
                 (1-iCO2CaptRate(allCy,PGALL,YTIME))*1e-3*iCo2EmiFac(allCy,"PG",PGEF,YTIME)*
                 (sum(NAP$NAPtoALLSBS(NAP,"PG"),VCarVal(allCy,NAP,YTIME))))
...
qLonMnmpowGenCost(allCy,PGALL,YTIME)$(TIME(YTIME)$(runCy(allCy)))..

         vLonMnmpowGenCost(allCy,PGALL,YTIME)
                 =E=
...
qLongPowGenIntPri(allCy,PGALL,ESET,YTIME)$(TIME(YTIME)$(runCy(allCy)))..

         vLongPowGenIntPri(allCy,PGALL,ESET,YTIME)
                 =E=
...
qShoPowGenIntPri(allCy,PGALL,ESET,YTIME)$(TIME(YTIME)$(runCy(allCy)))..

         vShoPowGenIntPri(allCy,PGALL,ESET,YTIME)
                 =E=
             sum(PGEF$PGALLTOEF(PGALL,PGEF),
                 (iVarCost(PGALL,YTIME)/1000+((
  SUM(EF,sum(WEF$EFtoWEF("PG",EF,WEF), iPriceFuelsInt(WEF,YTIME))*sTWhToMtoe/1000*1.5))$(not PGREN(PGALL))    +
...
QLongPowGenCost(allCy,ESET,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VLongAvgPowGenCost(allCy,ESET,YTIME)
                 =E=
         (
         SUM(PGALL, VElecProd(allCy,PGALL,YTIME)*VLongPowGenCost(allCy,PGALL,ESET,YTIME))

        +
         sum(CHP, VAvgElcProCHP(allCy,CHP,YTIME)*VChpElecProd(allCy,CHP,YTIME))
         )
/VElecDem(allCy,YTIME); 
...
QLonAvgPowGenCostNoClimPol(allCy,PGALL,ESET,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VLonAvgPowGenCostNoClimPol(allCy,PGALL,ESET,YTIME)
                 =E=
             ...
             / (1000*(7.25$ISET(ESET)+2.25$RSET(ESET))) +
             sum(PGEF$PGALLTOEF(PGALL,PGEF),
                 (iVarCost(PGALL,YTIME)/1000+((VFuelPriceSub(allCy,"PG",PGEF,YTIME)-iEffValueInEuro(allCy,"PG",ytime)/1000-iCo2EmiFac(allCy,"PG",PGEF,YTIME)*
...
QLonPowGenCostNoClimPol(allCy,ESET,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VLonPowGenCostNoClimPol(allCy,ESET,YTIME)
                 =E=
         (
         SUM(PGALL, (VElecProd(allCy,PGALL,YTIME))*VLonAvgPowGenCostNoClimPol(allCy,PGALL,ESET,YTIME))
...
QElecPriIndResNoCliPol(allCy,ESET,YTIME)$(TIME(YTIME)$(runCy(allCy)))..   !! The electricity price is based on previous year's production costs
        VElecPriIndResNoCliPol(allCy,ESET,YTIME)
                 =E=
        (1 + iVAT(allCy, YTIME)) *
        (
           (
             (VFuelPriceSub(allCy,"OI","ELC",YTIME-1)*sTWhToMtoe)$TFIRST(YTIME-1) +
             (
               VLonPowGenCostNoClimPol(allCy,"i",YTIME-1) 
              )$(not TFIRST(YTIME-1))
           )$ISET(ESET)
        +
           (
             (VFuelPriceSub(allCy,"HOU","ELC",YTIME-1)*sTWhToMtoe)$TFIRST(YTIME-1) +
             (
                VLonPowGenCostNoClimPol(allCy,"r",YTIME-1) 
             )$(not TFIRST(YTIME-1))
           )$RSET(ESET)
        );
...
qShortPowGenCost(allCy,ESET,YTIME)$(TIME(YTIME)$(runCy(allCy)))..

        vAvgPowerGenCostShoTrm(allCy,ESET,YTIME)
                 =E=
        (
        sum(PGALL,
        VElecProd(allCy,PGALL,YTIME)*
        ...
        +
         sum(CHP, VAvgVarProdCostCHP(allCy,CHP,YTIME)*VChpElecProd(allCy,CHP,YTIME))
         )
         /VElecDem(allCy,YTIME);
...
QPassCarsLft(allCy,DSBS,EF,YTIME)$(TIME(YTIME) $sameas(DSBS,"PC") $SECTTECH(DSBS,EF) $runCy(allCy))..
         VLifeTimeTech(allCy,DSBS,EF,YTIME)
                 =E=
         1/VScrRate(allCy,YTIME);
...
QGoodsTranspActiv(allCy,TRANSE,YTIME)$(TIME(YTIME) $TRANG(TRANSE) $runCy(allCy))..
         VGoodsTranspActiv(allCy,TRANSE,YTIME)
                 =E=
         (
          VGoodsTranspActiv(allCy,TRANSE,YTIME-1)
           ...
           * (VFuelPriceAvg(allCy,TRANSE,YTIME)/VFuelPriceAvg(allCy,TRANSE,YTIME-1))**iElastA(allCy,TRANSE,"c1",YTIME)
           * (VFuelPriceAvg(allCy,TRANSE,YTIME-1)/VFuelPriceAvg(allCy,TRANSE,YTIME-2))**iElastA(allCy,TRANSE,"c2",YTIME)
           * prod(kpdl,
                  [(VFuelPriceAvg(allCy,TRANSE,YTIME-ord(kpdl))/
                    VFuelPriceAvg(allCy,TRANSE,YTIME-(ord(kpdl)+1)))/
         ...
         (
           ...
           * (VGoodsTranspActiv(allCy,"GU",YTIME)/VGoodsTranspActiv(allCy,"GU",YTIME-1))**iElastA(allCy,TRANSE,"c4",YTIME)
...
QGapTranspActiv(allCy,TRANSE,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VGapTranspFillNewTech(allCy,TRANSE,YTIME)
                 =E=
         VNewReg(allCy,YTIME)$sameas(TRANSE,"PC")
         +
         (
         ( [VTrnspActiv(allCy,TRANSE,YTIME)/
         (sum((TTECH)$SECTTECH(TRANSE,TTECH),VLifeTimeTech(allCy,TRANSE,TTECH,YTIME-1))/TECHS(TRANSE))] +
          SQRT( SQR([VTrnspActiv(allCy,TRANSE,YTIME)/
          (sum((TTECH)$SECTTECH(TRANSE,TTECH),VLifeTimeTech(allCy,TRANSE,TTECH,YTIME-1))/TECHS(TRANSE))]) + SQR(1e-4) ) )/2
         )$(TRANP(TRANSE) $(not sameas(TRANSE,"PC")))
         +
         (
         ( [VGoodsTranspActiv(allCy,TRANSE,YTIME)/
         (sum((EF)$SECTTECH(TRANSE,EF),VLifeTimeTech(allCy,TRANSE,EF,YTIME-1))/TECHS(TRANSE))] + SQRT( SQR([VGoodsTranspActiv(allCy,TRANSE,YTIME)/
          (sum((EF)$SECTTECH(TRANSE,EF),VLifeTimeTech(allCy,TRANSE,EF,YTIME-1))/TECHS(TRANSE))]) + SQR(1e-4) ) )/2
         )$TRANG(TRANSE);
...
QSpecificFuelCons(allCy,TRANSE,TTECH,EF,YTIME)$(TIME(YTIME) $SECTTECH(TRANSE,EF) $TTECHtoEF(TTECH,EF) $runCy(allCy))..
         VSpecificFuelCons(allCy,TRANSE,TTECH,EF,YTIME)
                 =E=
         VSpecificFuelCons(allCy,TRANSE,TTECH,EF,YTIME-1) * prod(KPDL,
                     (
                        VFuelPriceSub(allCy,TRANSE,EF,YTIME-ord(KPDL))/VFuelPriceSub(allCy,TRANSE,EF,YTIME-(ord(KPDL)+1))
...
QTranspCostPerMeanConsSize(allCy,TRANSE,RCon,TTECH,YTIME)$(TIME(YTIME) $SECTTECH(TRANSE,TTECH) $(ord(Rcon) le iNcon(TRANSE)+1) $runCy(allCy))..
         VTranspCostPermeanConsSize(allCy,TRANSE,RCon,TTECH,YTIME)
         =E=
                       (
                         (
                           (iDisc(allCy,TRANSE,YTIME)*exp(iDisc(allCy,TRANSE,YTIME)*VLifeTimeTech(allCy,TRANSE,TTECH,YTIME)))
                           /
                           (exp(iDisc(allCy,TRANSE,YTIME)*VLifeTimeTech(allCy,TRANSE,TTECH,YTIME)) - 1)
                         ) * iCapCostTech(allCy,TRANSE,TTECH,YTIME)  * iCGI(allCy,YTIME)
                         + iFixOMCostTech(allCy,TRANSE,TTECH,YTIME)  +
                         (
                           (sum(EF$TTECHtoEF(TTECH,EF),VSpecificFuelCons(allCy,TRANSE,TTECH,EF,YTIME)*VFuelPriceSub(allCy,TRANSE,EF,YTIME)) )$(not PLUGIN(TTECH))
                           +
                           (sum(EF$(TTECHtoEF(TTECH,EF) $(not sameas("ELC",EF))),

                              (1-iShareAnnMilePlugInHybrid(allCy,YTIME))*VSpecificFuelCons(allCy,TRANSE,TTECH,EF,YTIME)*VFuelPriceSub(allCy,TRANSE,EF,YTIME))
                             + iShareAnnMilePlugInHybrid(allCy,YTIME)*VSpecificFuelCons(allCy,TRANSE,TTECH,"ELC",YTIME)*VFuelPriceSub(allCy,TRANSE,"ELC",YTIME)
                           )$PLUGIN(TTECH)
...
QTranspCostPerVeh(allCy,TRANSE,rCon,TTECH,YTIME)$(TIME(YTIME) $SECTTECH(TRANSE,TTECH) $(ord(rCon) le iNcon(TRANSE)+1) $runCy(allCy))..
         VTranspCostPerVeh(allCy,TRANSE,rCon,TTECH,YTIME)
         =E=
         VTranspCostPermeanConsSize(allCy,TRANSE,rCon,TTECH,YTIME)**(-4);
...
QTranspCostMatFac(allCy,TRANSE,RCon,TTECH,YTIME)$(TIME(YTIME) $SECTTECH(TRANSE,TTECH) $(ord(rCon) le iNcon(TRANSE)+1) $runCy(allCy))..
         VTranspCostMatFac(allCy,TRANSE,RCon,TTECH,YTIME) 
         =E=
         iMatrFactor(allCy,TRANSE,TTECH,YTIME) * VTranspCostPerVeh(allCy,TRANSE,rCon,TTECH,YTIME);
...
QTechSortVarCost(allCy,TRANSE,rCon,YTIME)$(TIME(YTIME) $(ord(rCon) le iNcon(TRANSE)+1) $runCy(allCy))..
         VTechSortVarCost(allCy,TRANSE,rCon,YTIME)
                 =E=
         sum((TTECH)$SECTTECH(TRANSE,TTECH), VTranspCostMatFac(allCy,TRANSE,rCon,TTECH,YTIME));
...
QTechSortVarCostNewEquip(allCy,TRANSE,TTECH,YTIME)$(TIME(YTIME) $SECTTECH(TRANSE,TTECH) $runCy(allCy))..
         VTechSortVarCostNewEquip(allCy,TRANSE,TTECH,YTIME)
         =E=
...
QConsEachTechTransp(allCy,TRANSE,TTECH,EF,YTIME)$(TIME(YTIME) $SECTTECH(TRANSE,TTECH) $TTECHtoEF(TTECH,EF) $runCy(allCy))..
         VConsEachTechTransp(allCy,TRANSE,TTECH,EF,YTIME)
                 =E=
         VConsEachTechTransp(allCy,TRANSE,TTECH,EF,YTIME-1) *
         (
                 (
                     ((VLifeTimeTech(allCy,TRANSE,TTECH,YTIME-1)-1)/VLifeTimeTech(allCy,TRANSE,TTECH,YTIME-1))
                 ...
                 +
                 (1 - VScrRate(allCy,YTIME))$sameas(TRANSE,"PC")
         )
         +
         VTechSortVarCostNewEquip(allCy,TRANSE,TTECH,YTIME) *
         (
                 VSpecificFuelCons(allCy,TRANSE,TTECH,EF,YTIME)$(not PLUGIN(TTECH))
                 +
                 ( ((1-iShareAnnMilePlugInHybrid(allCy,YTIME))*VSpecificFuelCons(allCy,TRANSE,TTECH,EF,YTIME))$(not sameas("ELC",EF))
                   + iShareAnnMilePlugInHybrid(allCy,YTIME)*VSpecificFuelCons(allCy,TRANSE,TTECH,"ELC",YTIME))$PLUGIN(TTECH)
         )/1000
         * VGapTranspFillNewTech(allCy,TRANSE,YTIME) *
         (
                 (
                 ...
                 +
                 (VTrnspActiv(allCy,TRANSE,YTIME))$sameas(TRANSE,"PC")
         );
...
QFinEneDemTranspPerFuel(allCy,TRANSE,EF,YTIME)$(TIME(YTIME) $SECTTECH(TRANSE,EF) $runCy(allCy))..
         VDemTr(allCy,TRANSE,EF,YTIME)
                 =E=
         sum((TTECH)$(SECTTECH(TRANSE,TTECH) $TTECHtoEF(TTECH,EF) ), VConsEachTechTransp(allCy,TRANSE,TTECH,EF,YTIME));
...
qFinEneDemTransp(allCy,TRANSE,YTIME)$(TIME(YTIME) $runCy(allCy))..
         vFinEneDemTranspSub(allCy,TRANSE,YTIME)
                 =E=
         sum(EF,VDemTr(allCy,TRANSE,EF,YTIME));
...
QMExtV(allCy,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VMExtV(allCy,YTIME)
                 =E=
         iTransChar(allCy,"RES_MEXTV",YTIME) * VMExtV(allCy,YTIME-1) *
...
QMExtF(allCy,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VMExtF(allCy,YTIME)
                 =E=
         iTransChar(allCy,"RES_MEXTF",YTIME) * iSigma(allCy,"S1") * EXP(iSigma(allCy,"S2") * EXP(iSigma(allCy,"S3") * VLamda(allCy,YTIME))) *
         VNumVeh(allCy,YTIME-1) / (iPop(YTIME-1,allCy) * 1000);
...
QNumVeh(allCy,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VNumVeh(allCy,YTIME)
                 =E=
         (VNumVeh(allCy,YTIME-1)/(iPop(YTIME-1,allCy)*1000) + VMExtF(allCy,YTIME) + VMExtV(allCy,YTIME)) *
...
QNewReg(allCy,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VNewReg(allCy,YTIME)
                 =E=
         (VMExtF(allCy,YTIME) + VMExtV(allCy,YTIME)) * (iPop(YTIME,allCy)*1000)  !! new cars due to GDP
         - VNumVeh(allCy,YTIME-1)*(1 - iPop(YTIME,allCy)/iPop(YTIME-1,allCy))    !! new cars due to population
         + VScrap(allCy,YTIME);                                                  !! new cars due to scrapping
...
QTrnspActiv(allCy,TRANSE,YTIME)$(TIME(YTIME) $TRANP(TRANSE) $runCy(allCy))..
         VTrnspActiv(allCy,TRANSE,YTIME)
                 =E=
         (  !! passenger cars
            VTrnspActiv(allCy,TRANSE,YTIME-1) *
           (VFuelPriceAvg(allCy,TRANSE,YTIME)/VFuelPriceAvg(allCy,TRANSE,YTIME-1))**iElastA(allCy,TRANSE,"b1",YTIME) *
           (VFuelPriceAvg(allCy,TRANSE,YTIME-1)/VFuelPriceAvg(allCy,TRANSE,YTIME-2))**iElastA(allCy,TRANSE,"b2",YTIME) *
           [(VNumVeh(allCy,YTIME-1)/(iPop(YTIME-1,allCy)*1000))/(VNumVeh(allCy,YTIME)/(iPop(YTIME,allCy)*1000))]**iElastA(allCy,TRANSE,"b3",YTIME) *
         ...
         (   !! other passenger transportation modes
           ...
           (VFuelPriceAvg(allCy,TRANSE,YTIME)/VFuelPriceAvg(allCy,TRANSE,YTIME-1))**iElastA(allCy,TRANSE,"c1",YTIME) *
           (VFuelPriceAvg(allCy,TRANSE,YTIME-1)/VFuelPriceAvg(allCy,TRANSE,YTIME-2))**iElastA(allCy,TRANSE,"c2",YTIME) *
           [(VNumVeh(allCy,YTIME)*VTrnspActiv(allCy,"PC",YTIME))/(VNumVeh(allCy,YTIME-1)*VTrnspActiv(allCy,"PC",YTIME-1))]**iElastA(allCy,TRANSE,"c4",YTIME) *
           prod(kpdl,
                  [(VFuelPriceAvg(allCy,TRANSE,YTIME-ord(kpdl))/
                    VFuelPriceAvg(allCy,TRANSE,YTIME-(ord(kpdl)+1)))/
                    (iCGI(allCy,YTIME)**(1/6))]**(iElastA(allCy,TRANSE,"c3",YTIME)*iFPDL(TRANSE,KPDL))
...
QScrap(allCy,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VScrap(allCy,YTIME)
                 =E=
         VScrRate(allCy,YTIME) * VNumVeh(allCy,YTIME-1);
...
QLevl(allCy,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VLamda(allCy,YTIME) !! level of saturation of gompertz function
                 =E=
         ( (VNumVeh(allCy,YTIME-1)/(iPop(YTIME-1,allCy)*1000)) / iPassCarsMarkSat(allCy) + 1 - SQRT( SQR((VNumVeh(allCy,YTIME-1)/(iPop(YTIME-1,allCy)*1000)) /  iPassCarsMarkSat(allCy) - 1)  ) )/2;
...
QScrRate(allCy,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VScrRate(allCy,YTIME)
                  =E=
...
QElecConsAll(allCy,DSBS,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VElecConsAll(allCy,DSBS,YTIME)
             =E=
         sum(INDDOM $SAMEAS(INDDOM,DSBS), VConsFuel(allCy,INDDOM,"ELC",YTIME)) + sum(TRANSE $SAMEAS(TRANSE,DSBS), VDemTr(allCy,TRANSE,"ELC",YTIME));
...
QElecConsNonSub(allCy,INDDOM,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VElecNonSub(allCy,INDDOM,YTIME)
                 =E=
         [
         VElecNonSub(allCy,INDDOM,YTIME-1) * ( iActv(YTIME,allCy,INDDOM)/iActv(YTIME-1,allCy,INDDOM) )**
         iElastNonSubElec(allCy,INDDOM,"a",YTIME)
         * ( VFuelPriceSub(allCy,INDDOM,"ELC",YTIME)/VFuelPriceSub(allCy,INDDOM,"ELC",YTIME-1) )**iElastNonSubElec(allCy,INDDOM,"b1",YTIME)
         * ( VFuelPriceSub(allCy,INDDOM,"ELC",YTIME-1)/VFuelPriceSub(allCy,INDDOM,"ELC",YTIME-2) )**iElastNonSubElec(allCy,INDDOM,"b2",YTIME)
         * prod(KPDL,
                  ( VFuelPriceSub(allCy,INDDOM,"ELC",YTIME-ord(KPDL))/VFuelPriceSub(allCy,INDDOM,"ELC",YTIME-(ord(KPDL)+1))
...
QConsOfRemSubEquip(allCy,DSBS,EF,YTIME)$(TIME(YTIME) $(not TRANSE(DSBS)) $SECTTECH(DSBS,EF) $runCy(allCy))..
         VConsRemSubEquip(allCy,DSBS,EF,YTIME)
                 =E=
         [
         (VLifeTimeTech(allCy,DSBS,EF,YTIME)-1)/VLifeTimeTech(allCy,DSBS,EF,YTIME)
         * (VFuelConsInclHP(allCy,DSBS,EF,YTIME-1) - (VElecNonSub(allCy,DSBS,YTIME-1)$(ELCEF(EF) $INDDOM(DSBS)) + 0$(not (ELCEF(EF) $INDDOM(DSBS)))))
         * (iActv(YTIME,allCy,DSBS)/iActv(YTIME-1,allCy,DSBS))**iElastA(allCy,DSBS,"a",YTIME)
         * (VFuelPriceSub(allCy,DSBS,EF,YTIME)/VFuelPriceSub(allCy,DSBS,EF,YTIME-1))**iElastA(allCy,DSBS,"b1",YTIME)
         * (VFuelPriceSub(allCy,DSBS,EF,YTIME-1)/VFuelPriceSub(allCy,DSBS,EF,YTIME-2))**iElastA(allCy,DSBS,"b2",YTIME)
         * prod(KPDL,
...
QDemSub(allCy,DSBS,YTIME)$(TIME(YTIME) $(not TRANSE(DSBS)) $runCy(allCy))..
         VDemSub(allCy,DSBS,YTIME)
                 =E=
         [
         VDemSub(allCy,DSBS,YTIME-1)
         * ( iActv(YTIME,allCy,DSBS)/iActv(YTIME-1,allCy,DSBS) )**iElastA(allCy,DSBS,"a",YTIME)
         * ( VFuelPriceAvg(allCy,DSBS,YTIME)/VFuelPriceAvg(allCy,DSBS,YTIME-1) )**iElastA(allCy,DSBS,"b1",YTIME)
         * ( VFuelPriceAvg(allCy,DSBS,YTIME-1)/VFuelPriceAvg(allCy,DSBS,YTIME-2) )**iElastA(allCy,DSBS,"b2",YTIME)
         * prod(KPDL,
                  ( (VFuelPriceAvg(allCy,DSBS,YTIME-ord(KPDL))/VFuelPriceAvg(allCy,DSBS,YTIME-(ord(KPDL)+1)))/(iCGI(allCy,YTIME)**(1/6))
...
qElecConsInd(allCy,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         vElecConsInd(allCy,YTIME)
         =E=
         SUM(INDSE,VElecNonSub(allCy,INDSE,YTIME));       
...
qDemInd(allCy,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
        vDemInd(allCy,YTIME)=E= SUM(INDSE,VDemSub(allCy,INDSE,YTIME));
...
QElecIndPrices(allCy,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VElecIndPrices(allCy,YTIME) =E=
        ( VElecIndPricesEst(allCy,YTIME) + sElecToSteRatioChp - SQRT( SQR(VElecIndPricesEst(allCy,YTIME)-sElecToSteRatioChp) + SQR(1E-4) ) )/2;
...
QFuelCons(allCy,DSBS,EF,YTIME)$(TIME(YTIME) $(not TRANSE(DSBS)) $SECTTECH(DSBS,EF) $(not HEATPUMP(EF)) $runCy(allCy))..
         VConsFuel(allCy,DSBS,EF,YTIME)
                 =E=
         VFuelConsInclHP(allCy,DSBS,EF,YTIME)$(not ELCEF(EF)) + 
         (VFuelConsInclHP(allCy,DSBS,EF,YTIME) + VElecConsHeatPla(allCy,DSBS,YTIME))$ELCEF(EF);
...
QElecIndPricesEst(allCy,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VElecIndPricesEst(allCy,YTIME)
                 =E=
         VElecIndPrices(allCy,YTIME-1) *
        ((VFuelPriceSub(allCy,"OI","ELC",YTIME-1)/VFuelPriceAvg(allCy,"OI",YTIME-1))/
        (VFuelPriceSub(allCy,"OI","ELC",YTIME-2)/VFuelPriceAvg(allCy,"OI",YTIME-2)))**(0.3) *
        ((VFuelPriceSub(allCy,"OI","ELC",YTIME-2)/VFuelPriceAvg(allCy,"OI",YTIME-2))/
        (VFuelPriceSub(allCy,"OI","ELC",YTIME-3)/VFuelPriceAvg(allCy,"OI",YTIME-3)))**(0.3);
...
QFuePriSubChp(allCy,DSBS,EF,YTIME)$(TIME(YTIME) $(not TRANSE(DSBS))  $SECTTECH(DSBS,EF) $runCy(allCy))..
        VFuePriSubChp(allCy,DSBS,EF,YTIME)
                =E=   
             (((VFuelPriceSub(allCy,DSBS,EF,YTIME) + (VRenValue(YTIME)/1000)$(not RENEF(EF))+iVarCostTech(allCy,DSBS,EF,YTIME)/1000)/iUsfEneConvSubTech(allCy,DSBS,EF,YTIME)- 
               (0$(not CHP(EF)) + (VFuelPriceSub(allCy,"OI","ELC",YTIME)*iFracElecPriChp*VElecIndPrices(allCy,YTIME))$CHP(EF)))  + SQRT( SQR(((VFuelPriceSub(allCy,DSBS,EF,YTIME)+iVarCostTech(allCy,DSBS,EF,YTIME)/1000)/iUsfEneConvSubTech(allCy,DSBS,EF,YTIME)- 
              (0$(not CHP(EF)) + (VFuelPriceSub(allCy,"OI","ELC",YTIME)*iFracElecPriChp*VElecIndPrices(allCy,YTIME))$CHP(EF))))  ) )/2;
...
QElecProdCosChp(allCy,DSBS,CHP,YTIME)$(TIME(YTIME) $INDDOM(DSBS) $runCy(allCy))..
         VElecProdCostChp(allCy,DSBS,CHP,YTIME)
                 =E=
                    ...
                    + sum(PGEF$CHPtoEF(CHP,PGEF), (VFuelPriceSub(allCy,"PG",PGEF,YTIME)+0.001*iCo2EmiFac(allCy,"PG",PGEF,YTIME)*
                         (sum(NAP$NAPtoALLSBS(NAP,"PG"),VCarVal(allCy,NAP,YTIME))))
                         * sTWhToMtoe /  (iBoiEffChp(allCy,CHP,YTIME) * VElecIndPrices(allCy,YTIME)) );        
...
QTechCost(allCy,DSBS,rCon,EF,YTIME)$(TIME(YTIME) $(not TRANSE(DSBS)) $(ord(rCon) le iNcon(DSBS)+1) $SECTTECH(DSBS,EF) $runCy(allCy))..
        VTechCost(allCy,DSBS,rCon,EF,YTIME) 
                 =E= 
                 VTechCostIntrm(allCy,DSBS,rCon,EF,YTIME)**(-iElaSub(allCy,DSBS)) ;   
...
QTechCostIntrm(allCy,DSBS,rCon,EF,YTIME)$(TIME(YTIME) $(not TRANSE(DSBS)) $(ord(rCon) le iNcon(DSBS)+1) $SECTTECH(DSBS,EF) $runCy(allCy))..
         VTechCostIntrm(allCy,DSBS,rCon,EF,YTIME) =E=
                  ( (( (iDisc(allCy,DSBS,YTIME)$(not CHP(EF)) + iDisc(allCy,"PG",YTIME)$CHP(EF)) !! in case of chp plants we use the discount rate of power generation sector
                       * exp((iDisc(allCy,DSBS,YTIME)$(not CHP(EF)) + iDisc(allCy,"PG",YTIME)$CHP(EF))*VLifeTimeTech(allCy,DSBS,EF,YTIME))
                     )
                      / (exp((iDisc(allCy,DSBS,YTIME)$(not CHP(EF)) + iDisc(allCy,"PG",YTIME)$CHP(EF))*VLifeTimeTech(allCy,DSBS,EF,YTIME))- 1)
                 ...
                 +
                  ( (( iDisc(allCy,DSBS,YTIME)
                       * exp(iDisc(allCy,DSBS,YTIME)*VLifeTimeTech(allCy,DSBS,EF,YTIME))
                     )
                      / (exp(iDisc(allCy,DSBS,YTIME)*VLifeTimeTech(allCy,DSBS,EF,YTIME))- 1)
                    ) * iCapCostTech(allCy,DSBS,EF,YTIME) * iCGI(allCy,YTIME)
                    +
                    iFixOMCostTech(allCy,DSBS,EF,YTIME)/1000
                    +
                    (
                      (VFuelPriceSub(allCy,DSBS,EF,YTIME)+iVarCostTech(allCy,DSBS,EF,YTIME)/1000)/iUsfEneConvSubTech(allCy,DSBS,EF,YTIME)
                    )
                    * iAnnCons(allCy,DSBS,"smallest") * (iAnnCons(allCy,DSBS,"largest")/iAnnCons(allCy,DSBS,"smallest"))**((ord(rCon)-1)/iNcon(DSBS))
                  )$NENSE(DSBS);  
...
QTechCostMatr(allCy,DSBS,rCon,EF,YTIME)$(TIME(YTIME) $(not TRANSE(DSBS)) $(ord(rCon) le iNcon(DSBS)+1) $SECTTECH(DSBS,EF) $runCy(allCy))..
        VTechCostMatr(allCy,DSBS,rCon,EF,YTIME) 
                                               =E=
        iMatrFactor(allCy,DSBS,EF,YTIME) * VTechCost(allCy,DSBS,rCon,EF,YTIME) ;
...
QTechSort(allCy,DSBS,rCon,YTIME)$(TIME(YTIME) $(not TRANSE(DSBS)) $(ord(rCon) le iNcon(DSBS)+1) $runCy(allCy))..
        VTechSort(allCy,DSBS,rCon,YTIME)
                        =E=
        sum((EF)$(SECTTECH(DSBS,EF) ),VTechCostMatr(allCy,DSBS,rCon,EF,YTIME));
...
QGapFinalDem(allCy,DSBS,YTIME)$(TIME(YTIME) $(not TRANSE(DSBS)) $runCy(allCy))..
         VGapFinalDem(allCy,DSBS,YTIME)
                 =E=
         VDemSub(allCy,DSBS,YTIME) - sum(EF$SECTTECH(DSBS,EF), VConsRemSubEquip(allCy,DSBS,EF,YTIME))
         + SQRT( SQR(VDemSub(allCy,DSBS,YTIME) - sum(EF$SECTTECH(DSBS,EF), VConsRemSubEquip(allCy,DSBS,EF,YTIME)))) /2;
...
QTechShareNewEquip(allCy,DSBS,EF,YTIME)$(TIME(YTIME) $SECTTECH(DSBS,EF) $(not TRANSE(DSBS)) $runCy(allCy))..
         VTechShareNewEquip(allCy,DSBS,EF,YTIME) =E=
         iMatrFactor(allCy,DSBS,EF,YTIME) / iCumDistrFuncConsSize(allCy,DSBS) *
         sum(rCon$(ord(rCon) le iNcon(DSBS)+1),
                  VTechCost(allCy,DSBS,rCon,EF,YTIME)
                  * iDisFunConSize(allCy,DSBS,rCon)/VTechSort(allCy,DSBS,rCon,YTIME));
...
QFuelConsInclHP(allCy,DSBS,EF,YTIME)$(TIME(YTIME) $(not TRANSE(DSBS)) $SECTTECH(DSBS,EF) $runCy(allCy))..
         VFuelConsInclHP(allCy,DSBS,EF,YTIME)
                 =E=
         VConsRemSubEquip(allCy,DSBS,EF,YTIME)+
         VTechShareNewEquip(allCy,DSBS,EF,YTIME)*VGapFinalDem(allCy,DSBS,YTIME)
         + (VElecNonSub(allCy,DSBS,YTIME))$(INDDOM(DSBS) and ELCEF(EF));
...
QVarProCostPerCHPDem(allCy,DSBS,CHP,YTIME)$(TIME(YTIME) $INDDOM(DSBS) $runCy(allCy))..
         VProCostCHPDem(allCy,DSBS,CHP,YTIME)
                 =E=
         iVarCostChp(allCy,DSBS,CHP,YTIME)/1E3
                    + sum(PGEF$CHPtoEF(CHP,PGEF), (VFuelPriceSub(allCy,"PG",PGEF,YTIME)+1e-3*iCo2EmiFac(allCy,"PG",PGEF,YTIME)*
                         (sum(NAP$NAPtoALLSBS(NAP,"PG"),VCarVal(allCy,NAP,YTIME))))
                         *sTWhToMtoe/(   iBoiEffChp(allCy,CHP,YTIME)*VElecIndPrices(allCy,YTIME)    ));
...
QAvgElcProCostCHP(allCy,CHP,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VAvgElcProCHP(allCy,CHP,YTIME)
         =E=

         (sum(INDDOM, VConsFuel(allCy,INDDOM,CHP,YTIME-1)/SUM(INDDOM2,VConsFuel(allCy,INDDOM2,CHP,YTIME-1))*VElecProdCostChp(allCy,INDDOM,CHP,YTIME)))
...
QAvgVarElecProd(allCy,CHP,YTIME)$(TIME(YTIME) $runCy(allCy)) ..
         VAvgVarProdCostCHP(allCy,CHP,YTIME)
         =E=

         (sum(INDDOM, VConsFuel(allCy,INDDOM,CHP,YTIME-1)/SUM(INDDOM2,VConsFuel(allCy,INDDOM2,CHP,YTIME-1))
         *VProCostCHPDem(allCy,INDDOM,CHP,YTIME)))
         $SUM(INDDOM2,VConsFuel.L(allCy,INDDOM2,CHP,YTIME-1))+0$(NOT SUM(INDDOM2,VConsFuel.L(allCy,INDDOM2,CHP,YTIME-1)));
...
QTotFinEneCons(allCy,EFS,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VFeCons(allCy,EFS,YTIME)
             =E=
         sum(INDDOM,
             sum(EF$(EFtoEFS(EF,EFS) $SECTTECH(INDDOM,EF) ), VConsFuel(allCy,INDDOM,EF,YTIME)))
         +
         sum(TRANSE,
             sum(EF$(EFtoEFS(EF,EFS) $SECTTECH(TRANSE,EF)), VDemTr(allCy,TRANSE,EF,YTIME)));
...
qTotFinEneConsAll(YTIME)$(TIME(YTIME))..
         vTotFinEneConsAll(YTIME) =E= sum((allCy,EFS), VFeCons(allCy,EFS,YTIME) );     
...
QFinNonEneCons(allCy,EFS,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VFNonEnCons(allCy,EFS,YTIME)
             =E=
...
QDistrLosses(allCy,EFS,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VLosses(allCy,EFS,YTIME)
             =E=
         (iRateLossesFinCons(allCy,EFS,YTIME) * (VFeCons(allCy,EFS,YTIME) + VFNonEnCons(allCy,EFS,YTIME)))$(not H2EF(EFS));  
...
QTranfOutputDHPlants(allCy,STEAM,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VTransfOutputDHPlants(allCy,STEAM,YTIME)
             =E=
         sum(DOMSE,
...
QRefCapacity(allCy,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VRefCapacity(allCy,YTIME)
             =E=
         [
         iResRefCapacity(allCy,YTIME) * VRefCapacity(allCy,YTIME-1)
         *
         (1$(ord(YTIME) le 16) +
         (prod(rc,
         (sum(EFS$EFtoEFA(EFS,"LQD"),VFeCons(allCy,EFS,YTIME-(ord(rc)+1)))/sum(EFS$EFtoEFA(EFS,"LQD"),VFeCons(allCy,EFS,YTIME-(ord(rc)+2))))**(0.5/(ord(rc)+1)))
         )
         $(ord(YTIME) gt 16)
...
QTranfOutputRefineries(allCy,EFS,YTIME)$(TIME(YTIME) $EFtoEFA(EFS,"LQD") $runCy(allCy))..
         VTransfOutputRefineries(allCy,EFS,YTIME)
             =E=
         [
         iResTransfOutputRefineries(allCy,EFS,YTIME) * VTransfOutputRefineries(allCy,EFS,YTIME-1)
         * (VRefCapacity(allCy,YTIME)/VRefCapacity(allCy,YTIME-1))**0.3
         * (
             1$(TFIRST(YTIME-1) or TFIRST(YTIME-2))
             +
             (
                sum(EF$EFtoEFA(EF,"LQD"),VFeCons(allCy,EF,YTIME-1))/sum(EF$EFtoEFA(EF,"LQD"),VFeCons(allCy,EF,YTIME-2))
...
QTransfInputRefineries(allCy,"CRO",YTIME)$(TIME(YTIME) $runCy(allCy))..
         VTransfInputRefineries(allCy,"CRO",YTIME)
             =E=
         [
         VTransfInputRefineries(allCy,"CRO",YTIME-1) *
         sum(EFS$EFtoEFA(EFS,"LQD"), VTransfOutputRefineries(allCy,EFS,YTIME)) /
         sum(EFS$EFtoEFA(EFS,"LQD"), VTransfOutputRefineries(allCy,EFS,YTIME-1))  ]$iRefCapacity(allCy,"%fStartHorizon%");                   
...
QTransfOutputNuclear(allCy,"ELC",YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VTransfOutputNuclear(allCy,"ELC",YTIME) =E=SUM(PGNUCL,VElecProd(allCy,PGNUCL,YTIME))*sTWhToMtoe;
...
QTransfInNuclear(allCy,"NUC",YTIME)$(TIME(YTIME)$(runCy(allCy)))..
        VTransfInNuclear(allCy,"NUC",YTIME) =E=SUM(PGNUCL,VElecProd(allCy,PGNUCL,YTIME)/iPlantEffByType(allCy,PGNUCL,YTIME))*sTWhToMtoe;
...
QTransfInPowerPls(allCy,PGEF,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VTransfInThermPowPls(allCy,PGEF,YTIME)
             =E=
        sum(PGALL$(PGALLtoEF(PGALL,PGEF)$((not PGGEO(PGALL)) $(not PGNUCL(PGALL)))),
             VElecProd(allCy,PGALL,YTIME) * sTWhToMtoe /  iPlantEffByType(allCy,PGALL,YTIME))
        +
        sum(PGALL$(PGALLtoEF(PGALL,PGEF)$PGGEO(PGALL)),
             VElecProd(allCy,PGALL,YTIME) * sTWhToMtoe) 
        +
        sum(CHP$CHPtoEF(CHP,PGEF),  sum(INDDOM,VConsFuel(allCy,INDDOM,CHP,YTIME))+sTWhToMtoe*VChpElecProd(allCy,CHP,YTIME))/(0.8+0.1*(ord(YTIME)-16)/32);
...
QTransfOutThermPP(allCy,TOCTEF,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VTransfOutThermPP(allCy,TOCTEF,YTIME)
             =E=
        (
             sum(PGALL$(not PGNUCL(PGALL)),VElecProd(allCy,PGALL,YTIME)) * sTWhToMtoe
             +
             sum(CHP,VChpElecProd(allCy,CHP,YTIME)*sTWhToMtoe)
         )$ELCEF(TOCTEF)
        +
        (                                                                                                         
          sum(INDDOM,
          sum(CHP$SECTTECH(INDDOM,CHP), VConsFuel(allCy,INDDOM,CHP,YTIME)))+
          iRateEneBranCons(allCy,TOCTEF,YTIME)*(VFeCons(allCy,TOCTEF,YTIME) + VFNonEnCons(allCy,TOCTEF,YTIME) + VLosses(allCy,TOCTEF,YTIME)) + 
          VLosses(allCy,TOCTEF,YTIME)                                                                                    
         )$STEAM(TOCTEF); 
...
QTotTransfInput(allCy,EFS,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VTotTransfInput(allCy,EFS,YTIME)
                 =E=
        (
            VTransfInThermPowPls(allCy,EFS,YTIME) + VTransfInputDHPlants(allCy,EFS,YTIME) + VTransfInNuclear(allCy,EFS,YTIME) +
             VTransfInputRefineries(allCy,EFS,YTIME)     !!$H2PRODEF(EFS)
        )$(not sameas(EFS,"OGS"))
        +
        (
          VTotTransfOutput(allCy,EFS,YTIME) - VFeCons(allCy,EFS,YTIME) - VFNonEnCons(allCy,EFS,YTIME) - iRateEneBranCons(allCy,EFS,YTIME)*
          VTotTransfOutput(allCy,EFS,YTIME) - VLosses(allCy,EFS,YTIME)
        )$sameas(EFS,"OGS");            
...
QTotTransfOutput(allCy,EFS,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VTotTransfOutput(allCy,EFS,YTIME)
                 =E=
         VTransfOutThermPP(allCy,EFS,YTIME) + VTransfOutputDHPlants(allCy,EFS,YTIME) + VTransfOutputNuclear(allCy,EFS,YTIME) +
         VTransfOutputRefineries(allCy,EFS,YTIME);        !!+ TONEW(allCy,EFS,YTIME)
...
QTransfers(allCy,EFS,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         ...
         (( (VTransfers(allCy,EFS,YTIME-1)*VFeCons(allCy,EFS,YTIME)/VFeCons(allCy,EFS,YTIME-1))$EFTOEFA(EFS,"LQD")+
          (
...
*' The outcome represents the gross inland consumption excluding the consumption of the specified energy branch in million tons of oil equivalent.
 QGrsInlConsNotEneBranch(allCy,EFS,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VGrsInlConsNotEneBranch(allCy,EFS,YTIME)
                 =E=
         VFeCons(allCy,EFS,YTIME) + VFNonEnCons(allCy,EFS,YTIME) + VTotTransfInput(allCy,EFS,YTIME) - VTotTransfOutput(allCy,EFS,YTIME) + VLosses(allCy,EFS,YTIME) - 
...
QGrssInCons(allCy,EFS,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VGrssInCons(allCy,EFS,YTIME)
                 =E=
         VFeCons(allCy,EFS,YTIME) + VEnCons(allCy,EFS,YTIME) + VFNonEnCons(allCy,EFS,YTIME) + VTotTransfInput(allCy,EFS,YTIME) - VTotTransfOutput(allCy,EFS,YTIME) +
          VLosses(allCy,EFS,YTIME) - VTransfers(allCy,EFS,YTIME);  
...
QPrimProd(allCy,PPRODEF,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VPrimProd(allCy,PPRODEF,YTIME)
                 =E=  [
         (
             iRatePriProTotPriNeeds(allCy,PPRODEF,YTIME) * (VGrsInlConsNotEneBranch(allCy,PPRODEF,YTIME) +  VEnCons(allCy,PPRODEF,YTIME))
         )$(not (sameas(PPRODEF,"CRO")or sameas(PPRODEF,"NGS")))
         +
         (
             iResHcNgOilPrProd(allCy,PPRODEF,YTIME) * VPrimProd(allCy,PPRODEF,YTIME-1) *
             (VGrsInlConsNotEneBranch(allCy,PPRODEF,YTIME)/VGrsInlConsNotEneBranch(allCy,PPRODEF,YTIME-1))**iNatGasPriProElst(allCy)
         )$(sameas(PPRODEF,"NGS") )
        +
         (
           iRatePriProTotPriNeeds(allCy,PPRODEF,YTIME) * VPrimProd(allCy,PPRODEF,YTIME-1) *
           ((VGrsInlConsNotEneBranch(allCy,PPRODEF,YTIME) + VExportsFake(allCy,PPRODEF,YTIME))/
            (VGrsInlConsNotEneBranch(allCy,PPRODEF,YTIME-1) + VExportsFake(allCy,PPRODEF,YTIME-1)))
         )$(sameas(PPRODEF,"NGS") )
         +(
...
QFakeExp(allCy,EFS,YTIME)$(TIME(YTIME) $IMPEF(EFS) $runCy(allCy))..
         VExportsFake(allCy,EFS,YTIME)
                 =E=
         (
...
QFakeImprts(allCy,EFS,YTIME)$(TIME(YTIME) $IMPEF(EFS) $runCy(allCy))..

         VFkImpAllFuelsNotNatGas(allCy,EFS,YTIME)

                 =E=
         (
            iRatioImpFinElecDem(allCy,YTIME) * (VFeCons(allCy,EFS,YTIME) + VFNonEnCons(allCy,EFS,YTIME)) + VExportsFake(allCy,EFS,YTIME)
         + iElecImp(allCy,YTIME)
         )$ELCEF(EFS)
         +
         (
            VGrssInCons(allCy,EFS,YTIME)+ VExportsFake(allCy,EFS,YTIME) + VConsFuel(allCy,"BU",EFS,YTIME)$SECTTECH("BU",EFS)
            - VPrimProd(allCy,EFS,YTIME)
         )$(sameas(EFS,"CRO"))
...
QNetImports(allCy,EFS,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VNetImports(allCy,EFS,YTIME)
                 =E=
         VFkImpAllFuelsNotNatGas(allCy,EFS,YTIME) - VExportsFake(allCy,EFS,YTIME);
...
QEneBrnchEneCons(allCy,EFS,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VEnCons(allCy,EFS,YTIME)
                 =E=
         iRateEneBranCons(allCy,EFS,YTIME) *
         (
           (
              VTotTransfOutput(allCy,EFS,YTIME) +
              VPrimProd(allCy,EFS,YTIME)$(sameas(EFS,"CRO") or sameas(EFS,"NGS"))
            )$(not TOCTEF(EFS))
            +
            (
              VFeCons(allCy,EFS,YTIME) + VFNonEnCons(allCy,EFS,YTIME) + VLosses(allCy,EFS,YTIME)
            )$TOCTEF(EFS)
         );                              
...
QCO2ElcHrg(allCy,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VCO2ElcHrgProd(allCy,YTIME)
         =E=
         sum(PGEF,sum(CCS$PGALLtoEF(CCS,PGEF),
                 VElecProd(allCy,CCS,YTIME)*sTWhToMtoe/iPlantEffByType(allCy,CCS,YTIME)*
...
QCumCO2Capt(allCy,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VCumCO2Capt(allCy,YTIME) =E= VCumCO2Capt(allCy,YTIME-1)+VCO2ElcHrgProd(allCy,YTIME-1);   
...
QWghtTrnstLinToExpo(allCy,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VWghtTrnstLnrToExpo(allCy,YTIME)
         =E=
         1/(1+exp(-iElastCO2Seq(allCy,"mc_s")*( VCumCO2Capt(allCy,YTIME)/iElastCO2Seq(allCy,"pot")-iElastCO2Seq(allCy,"mc_m")))); 
...
QCstCO2SeqCsts(allCy,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
         VCO2SeqCsts(allCy,YTIME) =E=
       (1-VWghtTrnstLnrToExpo(allCy,YTIME))*(iElastCO2Seq(allCy,"mc_a")*VCumCO2Capt(allCy,YTIME)+iElastCO2Seq(allCy,"mc_b"))+
       VWghtTrnstLnrToExpo(allCy,YTIME)*(iElastCO2Seq(allCy,"mc_c")*exp(iElastCO2Seq(allCy,"mc_d")*VCumCO2Capt(allCy,YTIME)));           
...
QTotGhgEmisAllCountrNap(NAP,YTIME)$(TIME(YTIME))..
        ...
        sum(allCy,
                ...
                +
                 sum(PGEF, VTransfInThermPowPls(allCy,PGEF,YTIME)*iCo2EmiFac(allCy,"PG",PGEF,YTIME)$(not h2f1(pgef))) !! input to power generation sector
                 +
                 sum(EFS, VTransfInputDHPlants(allCy,EFS,YTIME)*iCo2EmiFac(allCy,"PG",EFS,YTIME)) !! input to district heating plants
                 +
                 sum(EFS, VEnCons(allCy,EFS,YTIME)*iCo2EmiFac(allCy,"PG",EFS,YTIME)) !! consumption of energy branch
...
qHouseExpEne(allCy,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
                 vHouseExpEne(allCy,YTIME)
                 =E= 
                 SUM(DSBS$HOU(DSBS),SUM(EF$SECTTECH(dSBS,EF),VConsRemSubEquip(allCy,DSBS,EF,YTIME)*(VFuelPriceSub(allCy,DSBS,EF,YTIME)-iEffValueInEuro(allCy,DSBS,YTIME)/
                 1000-iCo2EmiFac(allCy,"PG",EF,YTIME)*sum(NAP$NAPtoALLSBS(NAP,"PG"),VCarVal(allCy,NAP,YTIME))/1000)))
                                          +VElecPriIndResNoCliPol(allCy,"R",YTIME)*VElecNonSub(allCy,"HOU",YTIME)/sTWhToMtoe;         
...
QFuelPriSubSepCarbVal(allCy,SBS,EF,YTIME)$(SECTTECH(SBS,EF) $TIME(YTIME) $(not sameas("NUC",EF)) $runCy(allCy))..
         VFuelPriceSub(allCy,SBS,EF,YTIME)
                 =E=
         (VFuelPriceSub(allCy,SBS,EF,YTIME-1) +
         ...
         +
         (
           ( VElecPriInduResConsu(allCy,"i",YTIME)$INDTRANS(SBS)+VElecPriInduResConsu(allCy,"r",YTIME)$RESIDENT(SBS))/sTWhToMtoe
            +
            ((iEffValueInEuro(allCy,SBS,YTIME))/1000)$DSBS(SBS)
...
QFuelPriSepCarbon(allCy,DSBS,EF,YTIME)$(SECTTECH(DSBS,EF) $TIME(YTIME) $runCy(allCy))..
        VFuelPriMultWgt(allCy,DSBS,EF,YTIME)
          =E= 
        iWgtSecAvgPriFueCons(allCy,DSBS,EF) * VFuelPriceSub(allCy,DSBS,EF,YTIME);
...
QAvgFuelPriSub(allCy,DSBS,YTIME)$(TIME(YTIME)$(runCy(allCy)))..
        VFuelPriceAvg(allCy,DSBS,YTIME)
                 =E=
         sum(EF$SECTTECH(DSBS,EF), VFuelPriMultWgt(allCy,DSBS,EF,YTIME));         
...
QElecPriIndResCons(allCy,ESET,YTIME)$(TIME(YTIME)$(runCy(allCy)))..  !! The electricity price is based on previous year's production costs
        VElecPriInduResConsu(allCy,ESET,YTIME)
                 =E=
        (1 + iVAT(allCy,YTIME)) *
        (
           (
             (VFuelPriceSub(allCy,"OI","ELC",YTIME-1)*sTWhToMtoe)$TFIRST(YTIME-1) +
             (
                VLongAvgPowGenCost(allCy,"i",YTIME-1)
              )$(not TFIRST(YTIME-1))
           )$ISET(ESET)
        +
           (
             (VFuelPriceSub(allCy,"HOU","ELC",YTIME-1)*sTWhToMtoe)$TFIRST(YTIME-1) +
             (
               VLongAvgPowGenCost(allCy,"r",YTIME-1) 
             )$(not TFIRST(YTIME-1))
           )$RSET(ESET)
        );
