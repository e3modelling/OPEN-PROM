...
QBaseLoad(runCy,YTIME)$TIME(YTIME)..
         VBaseLoad(runCy,YTIME)
             =E=
         (1/(1+exp(iBslCorrection(runCy,YTIME)*(VBsldEst(runCy,YTIME)-VBaseLoadMax(runCy,YTIME)))))*VBsldEst(runCy,YTIME)
        +(1-1/(1+exp(iBslCorrection(runCy,YTIME)*(VBsldEst(runCy,YTIME)-VBaseLoadMax(runCy,YTIME)))))*VBaseLoadMax(runCy,YTIME);
...
qCostHourProdInvCCS(runCy,PGALL,HOUR,YTIME)$(TIME(YTIME) $(CCS(PGALL) or NOCCS(PGALL))) ..
         ...
         =E=
          VCostHourProdInvDec(runCy,PGALL,HOUR,YTIME)**(-VSensCCS(runCy,YTIME));
...
QCostProdSpecTech(runCy,PGALL,YTIME)$(TIME(YTIME) $(CCS(PGALL) or NOCCS(PGALL)) ) ..
         ...
         =E=  
         sum(HOUR,VCostHourProdInvDec(runCy,PGALL,HOUR,YTIME)**(-VSensCCS(runCy,YTIME))) ;
...
QElecPeakLoads(runCy,YTIME)$TIME(YTIME)..
         ...
         =E= 
         sum(PGALL, VCostVarTechElec(runCy,PGALL,YTIME));     
