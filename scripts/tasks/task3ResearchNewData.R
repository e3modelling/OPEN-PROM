# Task 3: OPEN-PROM RESEARCH NEW DATA
# Two stages:
#  1. Material calibration (fScenario=4 hardcoded, GenerateInput=on).
#  2. Research solve (DevMode=0, GenerateInput=off, picks up OPENPROM_EXTRA_FLAGS).
# After both succeed, copy fresh data/ and targets/ back to the repo root, then reportOutput.
...
runTask3 <- function() {
  ...
  if (.Platform$OS.type == "unix") {
    calib_cmd <- paste(
      gams,
      "main.gms -o mainCalib.lst --WriteGDX=off --DevMode=0 --fScenario=4 --GenerateInput=on --Calibration=MatCalibration --CountrySolveMode=parallel",
      land_use_extra,
