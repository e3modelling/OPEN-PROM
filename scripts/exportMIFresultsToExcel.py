...
Script Description
------------------
This script converts a model reporting `.mif` file into a structured Excel report
with one worksheet per region. It extracts selected socioeconomic, emissions,
energy, and carbon-related indicators for predefined reporting years (2010–2100)
based on a fixed report structure.

Key features:
- Automatically loads `.mif` files with either semicolon or comma separators.
- Calculates non-CO2 emissions as:
      Emissions|nonCO2 = Emissions|Kyoto Gases - Emissions|CO2
- Detects the scenario name from the input file and uses it to name the output Excel file.
- Aggregates EU27 results by summing EU member states if an explicit EU27 entry is missing.
- Produces a standardized Excel workbook with one sheet per region and values
  organized by indicator and year.

The script is intended to transform raw model output into a clean reporting format
for scenario analysis and regional comparisons.
"""
